...
<c:if test="${fn:length(languages) > 1}">
	...
	<form:form action="${setLanguageActionUrl}" method="post" id="lang_form" class="lang-form">
		<%-- <spring:theme code="text.language" var="languageText"/> --%>
		<!-- <div class="control-group"> -->
	<%-- 	<label class="skip pull-left" style="width:40%;line-height:35px" for="lang_selector_${component.uid}">${languageText}</label> --%>
		
		<!-- <div class="controls"> -->
		<ycommerce:testId code="header_language_select">
			<select name="code" id="lang_selector" class="lang-selector selectpcker pull-left" data-width="100%">
			<%-- <option><spring:message code="loginPage.language"/></option> --%>
