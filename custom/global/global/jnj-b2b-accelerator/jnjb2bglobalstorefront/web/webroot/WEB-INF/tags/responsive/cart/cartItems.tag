					...
					<td headers="header4" class="quantity">
								...
								<c:when test="${not entry.product.multidimensional}" >
									<ycommerce:testId code="cart_product_quantity">
										...
										<c:if test="${entry.updateable}" >
											<form:input disabled="${not entry.updateable}" type="text" size="1" id="quantity" data-index="${loop.index}" class="qty" path="quantity"/>
										</c:if>
									</ycommerce:testId>
						...
						<c:if test="${entry.updateable}" >
							<ycommerce:testId code="cart_product_removeProduct">
								...
								<a href="#" id="RemoveProduct" data-index="${loop.index}" class="submitRemoveProduct">${iconCartRemove}</a>
							</ycommerce:testId>
						</c:if>
					</td>
