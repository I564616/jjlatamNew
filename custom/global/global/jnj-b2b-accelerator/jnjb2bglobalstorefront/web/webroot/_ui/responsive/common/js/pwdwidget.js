...
function _MakePWDWidget()
{
	...
	/*code += "<input type='password' class='pwdfield' name='"+pwdname+"' id='"+this.pwdfieldid+"' data-toggle='tooltip' data-placement='right' title='Password Strength: Your password must be 8 charactersn or longer and have two of the following four complexity classes: * Uppercase letters (A-Z)  * Lowercase letters (a-z)  * Numerals (0-9) * Non-alphabetic characters (such as !,#,$,%)'>";*/
...
function passwordStrength()
{
	...
	if (( password.match(/[a-z]/) ) && ( password.match(/[A-Z]/) ) && password.length >= 8 ) {score++;}

	if(password.match(/\d+/) && password.length >= 8 ){ score++;}
	if(password.match(/[A-Z]/) && password.length >= 8 ){ score++;}
	if(password.match(/[a-z]/) && password.length >= 8 ){ score++;}
	

	if(password.match(/[^a-z\d]+/) && password.length >= 8)	{score++};
	
	if(password.match(/[A-Z]/) && password.match(/\d+/) && password.length >= 8 ) {score=4;}
	if(password.match(/[a-z]/) && password.match(/[A-Z]/) && password.length >= 8 ) {score=4;}
	if(password.match(/[a-z]/) && password.match(/\d+/)  && password.length >= 8 ) {score=4;}
	
	if(password.match(/.[!,@,#,$,%,^,&,*,?,_,~]/) && password.match(/\d+/)  && password.length >= 8 ) {score=4;}
	if(password.match(/.[!,@,#,$,%,^,&,*,?,_,~]/) && password.match(/[a-z]/) && password.length >= 8 ) {score=4;}
	if(password.match(/.[!,@,#,$,%,^,&,*,?,_,~]/) && password.match(/[A-Z]/) && password.length >= 8 ) {score=4;}
