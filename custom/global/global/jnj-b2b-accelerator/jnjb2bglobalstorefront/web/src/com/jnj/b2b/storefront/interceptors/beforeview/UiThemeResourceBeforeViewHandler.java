 ...
 *  
 */
package com.jnj.b2b.storefront.interceptors.beforeview;
...
import com.jnj.b2b.storefront.util.CSRFTokenManager;
import com.jnj.b2b.storefront.web.view.UiExperienceViewResolver;
...
public class UiThemeResourceBeforeViewHandler implements BeforeViewHandler
{
	...
	protected static final String RecaptchaPublicKey="RecaptchaPublicKey";
	...
	public void beforeView(final HttpServletRequest request, final HttpServletResponse response, final ModelAndView modelAndView)
	{
		...
		modelAndView.addObject("hjid_Id", Config.getParameter("application.poll.hjid"));
		...
		//AAOL-4914 To get RecaptchaPublicKey in Js
		if(null!=currentSite.getDefaultCountry())
		{
			modelAndView.addObject(RecaptchaPublicKey, Config.getParameter(RECAPTCHA_PUBLIC_KEY_PROPERTY+currentSite.getDefaultCountry().getIsocode()));
			System.out.println(currentSite.getDefaultCountry().getIsocode()+"#####countrycode");
		}
		modelAndView.addObject("themeName", themeName);
		modelAndView.addObject("sessionPollInt", Config.getParameter("application.session.poll.interval"));  //sess_pollInterval
		modelAndView.addObject("sessionExpMin", Config.getParameter("application.session.expire.min")); //sess_expirationMinutes
		modelAndView.addObject("sessionWarnMin", Config.getParameter("application.session.warn.min")); //sess_warningMinutes
		modelAndView.addObject("isContract", Config.getParameter("country.contract.validation.available")); //isContract enable for country
		...
		for (final String addon : dependantAddOns)
		{
			modelAndView.addObject("addOnCommonResourcePath_" + addon, contextPath + "/_ui/addons/" + addon + "/"
					+ uiExperienceCodeLower + "/" + COMMON);
		}
	}
