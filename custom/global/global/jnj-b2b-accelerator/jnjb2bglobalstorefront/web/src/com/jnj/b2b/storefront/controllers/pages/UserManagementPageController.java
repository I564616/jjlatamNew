...
import com.jnj.b2b.storefront.forms.CustomerResetPasswordForm;
...
public class UserManagementPageController extends MyCompanyPageController
{
			...
			@RequestParam("user") final String user, final Model model) throws CMSItemNotFoundException
	{
		...
		// Handle paged search results
		final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
		final SearchPageData<B2BUserGroupData> searchPageData = b2bUserFacade.getPagedB2BUserGroupsForCustomer(
				pageableData, user);
		populateModel(model, searchPageData, showMode);
