 ...
 *  
 */
package com.jnj.b2b.storefront.web.theme;
...
public class StorefrontResourceBundleSource implements ThemeSource, ResourceLoaderAware, MessageSource
{
	...
	public static final String STOREFRONT_EXT_NAME = "jnjb2bglobalstorefront";
	...
	public void postConstruct() {
		final List<String> addOnNames = requiredAddOnsNameProvider.getAddOns(getExtensionName());
		lastMessageSource = createAddOnMessageSource(addOnNames, getParentMessageSource(), getBasePrefix());
	}
	...
	public Theme getTheme(final String themeName)
	{
		...
		synchronized (this.themeCache)
		{
            ...
            String extnName = getExtensionName();
			
			
			final List<String> addOnNames = requiredAddOnsNameProvider.getAddOns(extnName);
    ...
    private String getExtensionName() {
        String extnName = StringUtils.remove(appContext.getApplicationName(),"/");
		String servletContextName = ((ConfigurableWebApplicationContext) appContext).getServletContext().getServletContextName();
        if(StringUtils.isNotBlank(extnName) && extnName.equalsIgnoreCase("store")){
            extnName = STOREFRONT_EXT_NAME;
        }
		if(StringUtils.isBlank(extnName)) {
			extnName = StringUtils.isNotBlank(servletContextName) ? servletContextName : STOREFRONT_EXT_NAME;
		}
        return extnName;
    }
