 ...
 *
 */
package com.jnj.b2b.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
	...
	protected static final String SLASH_ISOCODE_EN = "/en";
	...
	private void setTargetUrl(final HttpServletRequest httpRequest)
	{
		...
		if (getUserService().isAnonymousUser(getUserService().getCurrentUser()))
		{
			...
			if (StringUtils.isNotEmpty(uri))
			{
				...
				List<String> values = JnJCommonUtil.getValues(Jnjb2bCoreConstants.SESSION_REDIRECTION_KEY,
				...
				{
					if (uri.contains(FOWARD_SLASH + value))
					{

						redirectUrl = uri.replace(contextPath, "");
						redirectUrl = redirectUrl.replace(SLASH_ISOCODE_EN, "");
						//redirectUrl = redirectUrl.substring(3);
						/** Setting the sales rep user as false by default **/
						if(null !=sessionService)
						{
							sessionService.setAttribute(Jnjb2bCoreConstants.JNJ_GT_TARGET_URL, redirectUrl);
						}
						break;
