...
public abstract class AbstractSearchPageController extends AbstractPageController
{
	...
	protected JnjPageableData createJnjPageableData(final int pageNumber, final int pageSize, final String sortCode,
												 final ShowMode showMode)
	{
	...
	{
		return getSiteConfigService().getInt("storefront.search.pageSize", 10);
	}
	...
	protected void setDisplayResult(final JnjGTSearchSortForm form,final long totalNumberOfResults,final int lastPageNumber){
		form.setTotalNumberOfResults(String.valueOf(totalNumberOfResults));
		form.setLastPageNumber(lastPageNumber);
