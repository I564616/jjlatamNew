 ...
 *  
 */
package com.jnj.b2b.test.orders;
...
import com.jnj.b2b.test.constants.Jnjb2bglobalTestConstants;
import com.jnj.b2b.test.services.AccountManagerApproveScheduleService;
...
public class B2BAcceleratorTestOrderData
{
	...
	@Deprecated
	private CheckoutFacade doNotUsecheckoutFacade;
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(Jnjb2bglobalTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(Jnjb2bglobalTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(Jnjb2bglobalTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(Jnjb2bglobalTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(Jnjb2bglobalTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(Jnjb2bglobalTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(Jnjb2bglobalTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(Jnjb2bglobalTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(Jnjb2bglobalTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(Jnjb2bglobalTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(Jnjb2bglobalTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(Jnjb2bglobalTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(Jnjb2bglobalTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(Jnjb2bglobalTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
	...
	protected String placeReplenishmentOrderInCancelledState(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			code = scheduleOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bglobalTestConstants.STANDARD_COSTCENTER, null, triggerData).getJobCode();
			if (code != null)
			{
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? Jnjb2bglobalTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderRequiringApproval(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bglobalTestConstants.STANDARD_COSTCENTER, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bglobalTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bglobalTestConstants.LOWLIMIT_COSTCENTER, null);
	...
	{
		// Lookup the site
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Jnjb2bglobalTestConstants.POWERTOOLS_SITE);
		// Lookup the customer
			...
			final String purchaseOrderNumber, final String costCenterCode, final String quotePetition)
	{
		...
		final ImpersonationContext ctx = new ImpersonationContext();
		ctx.setSite(getCmsAdminSiteService().getSiteForId(Jnjb2bglobalTestConstants.POWERTOOLS_SITE));
		ctx.setUser(customer);
				...
				new ImpersonationService.Executor<Object, ImpersonationService.Nothing>()
				{
					...
					public Object execute() throws ImpersonationService.Nothing
					{
								...
								quotePetition, customer))
						{
							try
							{
								orderData = doNotUsecheckoutFacade.placeOrder();
			...
			final B2BCustomerModel customer)
	{
		LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(),
				Jnjb2bglobalTestConstants.POWERTOOLS_SITE));
		...
		if (containsOrder(orderList, purchaseOrderNumber))
		{
			LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ",
					purchaseOrderNumber, customer.getUid(), Jnjb2bglobalTestConstants.POWERTOOLS_SITE));
		...
		else
		{
			...
			if (costCenterCode == null)
			{
				...
				paymentType.setCode(CheckoutPaymentType.CARD.getCode());

				cartData.setPaymentType(paymentType);
				doNotUsecheckoutFacade.setPaymentInfoIfAvailable();
				doNotUsecheckoutFacade.authorizePayment("123");
			...
			else
			{
				...
				if (quotePetition != null)
				{
					...
					b2BComment.setComment(quotePetition);
			...
			doNotUsecheckoutFacade.setDeliveryModeIfAvailable();
	...
	{
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Jnjb2bglobalTestConstants.POWERTOOLS_SITE);
		final B2BCustomerModel customer = getUserService().getUserForUID(Jnjb2bglobalTestConstants.APPROVER_UID.toLowerCase(),
	...
	@Required
	public void setB2bCheckoutFacade(final de.hybris.platform.b2bacceleratorfacades.api.cart.CheckoutFacade b2bCheckoutFacade)
	{
		this.b2bCheckoutFacade = b2bCheckoutFacade;
	}

	@Required
	public void setDoNotUsecheckoutFacade(final CheckoutFacade doNotUsecheckoutFacade)
	{
		this.doNotUsecheckoutFacade = doNotUsecheckoutFacade;
	}
