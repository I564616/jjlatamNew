...
import com.jnj.core.data.JnjGTCreateConsignmentResponseData;
 ...
 *
 */
public class DefaultJnjGTCreateConsignmentOrderMapper implements JnjGTCreateConsignmentOrderMapper
{
	private static final Logger LOGGER = Logger.getLogger(DefaultJnjGTCreateConsignmentOrderMapper.class);
	 ...
	 */
	@Override
	public JnjGTCreateConsignmentResponseData mapCreateConsignmentOrderRequestResponse(final OrderModel orderModel,
			final JnjGTSapWsData sapWsData) throws IntegrationException, SystemException, BusinessException
	{
		...
		final JnjGTCreateConsignmentResponseData jnjGTCreateConsignmentResponseData = new JnjGTCreateConsignmentResponseData();
...
//		orderCreationInSAPInput, sapWsData);
				...
				&& StringUtils.equals(JnjOrderTypesEnum.ZKB.getCode(), orderModel.getOrderType().getCode()))
		{
			...
			if (jnjGTOrderChangeResponseData.isSapResponseStatus())
			{
				...
				jnjGTCreateConsignmentResponseData.setSavedSuccessfully(jnjCartService.saveCartModel(orderModel, true));
				jnjGTCreateConsignmentResponseData.setSapResponseStatus(true);
				//AAOL-2420 changes
				jnjGTCreateConsignmentResponseData.setCreditLimitCode(Config.getParameter("test.credit.limit.message.code"));
				//end of AAOL-2420 changes
			}
			else
			{
				jnjGTCreateConsignmentResponseData.setSapResponseStatus(false);
				jnjGTCreateConsignmentResponseData.setErrorMessage(jnjGTOrderChangeResponseData.getErrorMessage());
			}
		}
		...
		else if (null != orderCreationInSAPOutput && null != orderCreationInSAPOutput.getErrorMessage())
		{
			...
			mapOrderModelFromOutOrderLine(orderModel, orderCreationInSAPOutput);
			jnjGTCreateConsignmentResponseData.setSapResponseStatus(false);
			jnjGTCreateConsignmentResponseData.setErrorMessage(orderCreationInSAPOutput.getErrorMessage().getValue());
		}
		...
		jnjGTCreateConsignmentResponseData.setCreditLimitCode(Config.getParameter("test.credit.limit.message.code"));
		//end of AAOL-2420 changes
		return jnjGTCreateConsignmentResponseData;
	}
