...
/**
 * Removes the record of B2BUnit
 * 
 * @author Accenture
 * @version 1.0
 * 
 */
public class JnjGTB2BUnitRemoveInterceptor implements RemoveInterceptor
{
	...
	public void onRemove(final Object model, final InterceptorContext arg1) throws InterceptorException
	{
		...
		if (model instanceof JnJB2BUnitModel)
		{
			...
			if (modelService.isNew(JnjGTB2BUnit))
			{
				...
				this.logAuditData(null, "New B2BUnit Remove event. User created is: " + JnjGTB2BUnit.getUid(), null, true, false,
						new Date());
			...
			else
			{
				...
				this.logAuditData(null, "Existing B2BUnit Remove Event. ", null, true, false, new Date());
