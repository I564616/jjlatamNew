...
public class JnjGTBaseMaterialNumberValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider,
		Serializable
{
			...
			final Object model) throws FieldValueProviderException
	{
		...
		String materialBaseProductCode;

		// If materialBaseProduct is null, index the product code
		if (naProductModel.getMaterialBaseProduct() == null)
		{
			materialBaseProductCode = naProductModel.getCode();
		}
		// If materialBaseProduct is not null, index the materialBaseProduct's code
		else
		{
			materialBaseProductCode = naProductModel.getMaterialBaseProduct().getCode();
		}
		final Collection<FieldValue> fieldValues = new ArrayList<FieldValue>();

		fieldValues.addAll(createFieldValue(materialBaseProductCode, indexedProperty));

		return fieldValues;

	}

	protected List<FieldValue> createFieldValue(final String materialBaseProductCode, final IndexedProperty indexedProperty)
	{
		...
		{
			fieldValues.add(new FieldValue(fieldName, materialBaseProductCode));
		}
		return fieldValues;
	}
