 ...
 * @version 1.0
 */
public class JnjGTUserGroupPrepareInterceptor implements PrepareInterceptor
{
	...
	public void onPrepare(final Object model, final InterceptorContext arg1) throws InterceptorException
	{
		if (model instanceof UserGroupModel && !(model instanceof B2BUnitModel))
		{
			...
			if (modelService.isNew(JnjGTUserGroup))
			{
				...
				{
					LOG.debug("going to log event for create new user group");
				}
				this.logAuditData(null, "New User modify event. User created is: " + JnjGTUserGroup.getUid(), null, true, false,
						new Date());
			}
			else if (userGroups.contains(JnjGTUserGroup.getUid()))
			{
				...
				this.logAuditData(null, "Existing usergroup modify Event. ", null, true, false, new Date());
			}
		}
	}
