...
/**
 * This class represents the email context for the successful registration flow.
 * 
 * @author Accenture
 * @version 1.0
 */
public class JnjGTSuccessfulRegistrationEmailContext extends CustomerEmailContext
{
	...
	public void init(final StoreFrontCustomerProcessModel storeFrontCustomerProcessModel, final EmailPageModel emailPageModel)
	{
		...
		if (storeFrontCustomerProcessModel instanceof JnjGTSuccessfulRegistrationEmailProcessModel)
		{
			...
			put(FROM_EMAIL, Config.getParameter(Jnjb2bCoreConstants.Register.REGISTRATION_FROM_EMAIL_ADDRESS));
			

			/** Setting the From User display name **/
			put(FROM_DISPLAY_NAME,
					jnjGTSuccessfulRegistrationProcessModel.getJnjGTRegistrationDetails().get(USER_FIRST_NAME)
							+ (null != jnjGTSuccessfulRegistrationProcessModel.getJnjGTRegistrationDetails().get(USER_LAST_NAME) ? Jnjb2bCoreConstants.SPACE
									+ jnjGTSuccessfulRegistrationProcessModel.getJnjGTRegistrationDetails().get(USER_LAST_NAME)
									: ""));
		}
