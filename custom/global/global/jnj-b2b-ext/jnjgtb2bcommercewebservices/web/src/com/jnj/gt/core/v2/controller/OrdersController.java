...
import com.jnj.gt.core.exceptions.PaymentAuthorizationException;
...
public class OrdersController extends BaseCommerceController
{
	...
	@ResponseBody
	@Operation(summary = "Get a order", description = "Returns details of a specific order based on the order GUID (Globally Unique Identifier) or the order CODE. The response contains detailed order information.")
	@ApiBaseSiteIdParam
	...
	@ResponseBody
	@Operation(summary = "Get a order", description = "Returns specific order details based on a specific order code. The response contains detailed order information.")
	...
	@ResponseBody
	@Operation(summary = "Get order history for user", description = "Returns order history data for all orders placed by a specified user for a specified base store. The response can display the results across multiple pages, if required.")
	@ApiBaseSiteIdAndUserIdParam
	...
	@ResponseBody
	@Operation(summary = "Get total number of orders", description = "In the response header, the \"x-total-count\" indicates the total number of orders placed by a specified user for a specified base store.")
	@ApiBaseSiteIdAndUserIdParam
	...
	@ResponseBody
	@Operation(summary = "Post a order", description = "Authorizes the cart and places the order. The response contains the new order data.")
	@ApiBaseSiteIdAndUserIdParam
	@SuppressWarnings("squid:S1160")
	public OrderWsDTO placeOrder(
			@Parameter(description = "Cart code for logged in user, cart GUID for guest checkout", required = true) @RequestParam(required = true) final String cartId, //NOSONAR
			@Parameter(description = "CCV security code.") @RequestParam(required = false) final String securityCode,
			@Parameter(description = "Response configuration. This is the list of fields that should be returned in the response body.", schema = @Schema(allowableValues = {"BASIC, DEFAULT, FULL"})) @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
