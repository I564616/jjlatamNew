...
import com.jnj.gt.core.product.data.PromotionDataList;
...
public class PromotionsController extends BaseController
{
	...
	@Operation(summary = "Get a list of promotions", description = "Returns promotions defined for a current base site. Requests pertaining to promotions have been developed "
	...
	public PromotionListWsDTO getPromotions(
			@Parameter(description = "Defines what type of promotions should be returned. Values supported for that parameter are: <ul><li>all: All available promotions are "
					+ "returned</li><li>product: Only product promotions are returned</li><li>order: Only order promotions are returned</li></ul>", schema = @Schema(allowableValues = {"all, product, order"}), required = true) @RequestParam final String type,
			@Parameter(description = "Only promotions from this group are returned", required = false) @RequestParam(required = false) final String promotionGroup,
	...
	@ResponseBody
	@Operation(summary = "Get a promotion based on code", description = "Returns details of a single promotion specified by a promotion code. Requests pertaining to "
	...
	@ApiBaseSiteIdParam
	public PromotionWsDTO getPromotionByCode(@Parameter(description = "Promotion identifier (code)") @PathVariable final String code,
			@Parameter(description = "Response configuration. This is the list of fields that should be returned in the response body.", schema = @Schema(allowableValues = {"BASIC, DEFAULT, FULL"})) @RequestParam(defaultValue = "BASIC") final String fields)
	{
