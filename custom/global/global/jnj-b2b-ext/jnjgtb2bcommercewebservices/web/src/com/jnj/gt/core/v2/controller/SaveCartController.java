...
public class SaveCartController extends BaseCommerceController
{
	...
	@ResponseBody
	@Operation(summary = "Explicitly saves a cart.", description = "Explicitly saves a cart.")
	...
	public SaveCartResultWsDTO saveCart(
			...
			@Parameter(description = "The name that should be applied to the saved cart.", required = false) @RequestParam(value = "saveCartName", required = false) final String saveCartName,
			@Parameter(description = "The description that should be applied to the saved cart.", required = false) @RequestParam(value = "saveCartDescription", required = false) final String saveCartDescription,
	...
	@ResponseBody
	@Operation(summary = "Restore a saved cart.", description = "Restore a saved cart.")
	@ApiBaseSiteIdAndUserIdParam
	...
	@ResponseBody
	@Operation(summary = "Flag a cart for deletion.", description = "Flags a cart for deletion (the cart doesn't have corresponding save cart attributes anymore). The cart is not "
	...
	@ResponseBody
	@Operation(summary = "Get a saved cart.", description = "Returns a saved cart for an authenticated user. The cart is identified using the \"cartId\" parameter.")
	@ApiBaseSiteIdAndUserIdParam
	...
	@ResponseBody
	@Operation(summary = "Explicitly clones a cart.", description = "Explicitly clones a cart.")
	@ApiBaseSiteIdAndUserIdParam
	public SaveCartResultWsDTO cloneSaveCart(
			@Parameter(description = "Cart identifier: cart code for logged in user, cart guid for anonymous user, 'current' for the last modified cart", required = true) @PathVariable final String cartId,
			@Parameter(description = "The name that should be applied to the cloned cart.", required = false) @RequestParam(value = "name", required = false) final String name,
			@Parameter(description = "The description that should be applied to the cloned cart.", required = false) @RequestParam(value = "description", required = false) final String description,
			@Parameter(description = "Response configuration. This is the list of fields that should be returned in the response body.", schema = @Schema(allowableValues = {"BASIC, DEFAULT, FULL"})) @RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields)
			throws CommerceSaveCartException
	{
		final CommerceSaveCartParameterData parameters = new CommerceSaveCartParameterData();
