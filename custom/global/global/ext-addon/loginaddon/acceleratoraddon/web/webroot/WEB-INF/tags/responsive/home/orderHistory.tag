...
<div id="tables" class="orderstatuspage">
	<div class="row ordertextmargin">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 orderStatus text-left"><spring:message code="text.account.orderHistory.orderStatus"/></div>
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<button class="btn btnclsactive pull-right">
	...
	<c:choose>
		<c:when test="${searchPageData.results eq null  || empty searchPageData.results}">
			<div class="visible-xs"style="padding: 20px;font-size: 20px;background: white;"><spring:message	code="homePage.orderHistory.nodata" /></div>
		</c:when>
		<c:otherwise>
						...
						<div class="swiper-slide mobiletable">
							...
							<table class="table table-bordered">
								<tbody>
									...
									<tr>
										<th><spring:message code="orderHistoryPage.tracking" /></th>
										<td><c:choose>
												<c:when test="${not empty order.channel}">
													
													<a href="#">${order.channel}</a>
												</c:when>
												<c:otherwise>
	...
	<!--  mobile slider -->
	<div class="desktoptableparent" id="orderTableFixedHeader">
		<table id="ordersTablehome" class="table table-bordered table-striped ">
			<thead>
				<tr>
				...
				<th class="no-sort"><spring:message
						code="orderHistoryPage.tracking" /></th>
				<th class="no-sort" id="channel" onclick="toggleSort(id)"><spring:message
						code="orderHistoryPage.channel" /></th>
				<th class="no-sort" id="status" onclick="toggleSort(id)"><spring:message
							code="orderHistoryPage.status" /></th>
				<!-- Adding for AAOL-3469 start-->			
					<th class="no-sort text-right" id="total" onclick="toggleSort(id)">
	...
	<%-- </div>
	<div class="shomoreandviewall col-xs-12 whitebg">
