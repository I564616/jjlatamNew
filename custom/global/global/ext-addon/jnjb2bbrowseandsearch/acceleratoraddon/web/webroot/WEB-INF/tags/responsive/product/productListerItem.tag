 ...
 <product:productPrimaryImage product="${product}" format="thumbnail" />
		                ...
		                <c:otherwise>
		                	<c:choose>
		                		<c:when test="${product.obsolete}">
		                			|<span class="clrDisable"><label:message messageCode="category.result.obsolete"/></span>
		                		</c:when>
		                		<c:otherwise>
		...
		<td class="paddingright40px" width="150px">
			...
			<div class="row">
				...
				<!-- added addtocartqty class for AAOL-5157 Fix  -->
				<input type="text" value="0" class="addtocartqty halign2 column3Input pull-right descMid form-control txtWidth" id="qty_desktop_${index}" title="check" maxlength="6" name="qty" ${disabledProduct}/>
			   </form:form>
			</div>
			<div></div>
			<div class="row">
