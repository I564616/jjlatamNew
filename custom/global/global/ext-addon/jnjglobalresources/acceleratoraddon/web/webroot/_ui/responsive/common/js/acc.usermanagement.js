...
$("#createProfileButton").click(function(){
	...
	dataObj.email = emailAddress;
	{
		jQuery.ajax({
			...
			success: function (data) {
				if(!data){
					if($("#createNewProfileForm").valid() && validateRolesAndGroupsFields()){
		...
		jQuery.ajax({
			...
			success: function (data) {
				$.each(data,function(type,content){
					if(type=="SUCCESS"){
						$("#myTrainingResources").find(".success").first().html("<label class='correctAnswer mar0'>" + content + "</label>").show();	
					} else  if (type=="FAIL"){
						$("#myTrainingResources").find(".registerError").first().html("<label class='error'>" + content + "</label>").show();
					} 
				})		
			},
			error: function (e) {
...
$(".updateProfileButton").click(function(){
	...
	if(emailAddress == existingEmail ){
		if($("#editUserProfileForm").valid() && validateRolesAndGroupsFields()){
...
function validateRolesAndGroupsFields(){
	...
	if(!roleFlag){
		$(".roleContent").find('div.registerError').html(REGISTER_ROLE);
	}
	...
	if(!accountFlag){
		$(".multipleAccountContent").find('div.registerError').html(REGISTER_ACCOUNT);
	}
