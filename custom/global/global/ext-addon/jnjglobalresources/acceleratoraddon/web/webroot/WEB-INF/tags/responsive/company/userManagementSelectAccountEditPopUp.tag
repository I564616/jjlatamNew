...
<div class="modal fade jnj-popup" id="changeAcntPopup" role="dialog">
			...
			<input type="hidden" id="totalResultsAccountSelection" value="${accountPaginationData.totalNumberOfResults}" />
				...
				<div class="modal-content" id="ContractDetailsPage">
					...
					<div class="modal-body" style="height:122px">
					...
					<div class="modalbody contract-detail-desktab"  style="overflow-y: auto;max-height: 300px;">
						...
						<%-- 	<c:forEach items="${accountToAdded}" var="cuurentId" varStatus="count">
							
									
									</c:forEach> --%>
									
		 <c:set value="${(currentAccountIds.uid eq accountsObj.key) ? 'checked' : ''}" var="selectedAccount" />
				         <input type="checkbox" class="accountSelectionUM accountSelection" id="accountSelection_${count.count}" value="${accountsObj.key}" data="${accountsObj.value}" checked/>
					...
					<div class="btn-mobile padding25">
						...
						<button type="submit" class="btn btnclsactive searchbtn pull-right width20percent" id="accountsSubmitUM">OK</button>
