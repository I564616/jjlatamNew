		...
		<div
			...
			<!-- 	<div class="row bottom-border"> -->
				...
				<div class="row col-lg-12 col-md-12 col-sm-12 col-xs-12 shippingAddressDetails">
					<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12 shippingAddressNA leftSidePanel" id="order-detail-left-panel">
						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 leftSidePanelLink linkspadding" id="orderDetail-leftLink">
					...
					<div class="col-lg-7 col-md-7 col-sm-7 col-xs-12  orderDetailsAndStatus rightSidePanel"  id="order-detail-right-panel" >
						...
						<div class="col-lg-7 col-md-7 col-sm-7 col-xs-12 TextPadding">
								...
								<c:if test="${isMddSite && not empty orderData.dropShipAccount  && 'ZDEL' eq orderData.orderType}">
									<h6>
										<spring:message code="order.detail.shipTo.Acc" />
									</h6>
								...
								<c:if test="${isMddSite && not empty orderData.surgeonName  && 'ZDEL' eq orderData.orderType}">
									<h6>
										<spring:message code="order.detail.surgon.name" />
									</h6>
									<div>${orderData.surgeonName}
									<p>
							...
							<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12 TextPadding">
								...
								<c:if test="${isMddSite}">
								 ...
								 <c:if test="${not empty orderData.requestedDeliveryDate}">
								 	<h6>
										...
										<!-- Jira#3647 -->
								<c:if test="${not empty orderData.dropShipAccount && not 'ZDEL' eq orderData.orderType}">
										<h6>
											<spring:message
								...
								<c:if test="${not empty orderData.distributorPONumber}">
									<h6>
										<spring:message code="order.detail.distributor.po" />
									</h6>
									<p>${orderData.distributorPONumber}</p>						
								
								</c:if>
								<c:if test="${not empty orderData.orderedBy}">
									<h6>
										<spring:message code="orderDetailPage.orderData.orderBy" />
									</h6>
									<p>${orderData.orderedBy}</p>
								</c:if>
							</div>
						</div>
						...
						<div class="">
							<c:if
								test="${isMddSite && orderData.attachDocURL!= null && 'ZDEL' eq orderData.orderType}">
								<a target="_new" href="${orderData.attachDocURL}"><spring:message
										code="orderDetailPage.orderData.downloadDeliveredOrderForm" /></a>
								<span class="pipesymbol">|</span>
							</c:if>
							<c:if
								test="${isMddSite && orderData.signature!= null && 'ZDEL' eq orderData.orderType}">
								
									<span><a target="_new" href="${orderData.signature}"><spring:message
											code="orderDetailPage.orderData.downloadsignature" /></a>
								</span>
							</c:if>
							<c:if
								test="${isMddSite && orderData.signature== null && 'ZDEL' eq orderData.orderType}">
									<span><a title='No signature captured'><spring:message
											code="orderDetailPage.orderData.downloadsignature" /></a></span>
							</c:if>
							<span class="pipesymbol">|</span> <a><spring:message
