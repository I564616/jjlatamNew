...
<%@ taglib prefix="fmt" uri="jakarta.tags.fmt"%>
	...
	<tr>
					...
					<c:choose>
						<c:when test="${isMddSite}">
							...
							<c:if test="${not empty entry.hazmatInd}">
								<div>
									<p><spring:message code='orderDetails.hazmatCode'/>: ${entry.hazmatInd}
								</div>
							</c:if>
							<c:if test="${not empty entry.specialStockPartner}">
								<div>
									<p><spring:message code='orderDetails.specialStockPartner'/>: ${entry.specialStockPartner}
								</div>
							</c:if>
							</br>
							<c:if test="${not empty entry.lotNumber}">
								<p>
									<span class="smallFont"><spring:message code='cart.return.entries.lotNumber'/>:</span></br> <span
					...
					<c:choose>
						<c:when test="${isMddSite}">
							...
							<c:if test="${not empty entry.priceOverrideReason}">
								<p>
									<span class="smallFont"><spring:message code="cart.common.price" /></span></br><span
										class="txtFont">${entry.priceOverrideReason}</span>
								</p>
							</c:if>
							<c:if test="${not empty entry.approver}">
								<p>
									<span class="smallFont"><spring:message code="product.detail.basic.approver" /></span></br> <span class="txtFont">${entry.approver}</span>
								</p>

							</c:if>
							<c:if test="${not empty entry.expectedShipDate and not entry.expandableSchedules}">
								<p>
									<span class="labelText"><spring:message code="product.detail.basic.estimated" /></span></br><span
										class="txtFont"><c:set var="dateformat"> <spring:message code='date.dateformat'/></c:set><fmt:formatDate
											value="${entry.expectedShipDate}" pattern="${dateformat}" /></span>
								</p>
							</c:if>
							<c:if test="${not empty entry.shipmentLocation}">
								<p>
									<span class="labelText"><spring:message code="product.detail.basic.shipment" /></span></br> <span
						...
						<c:otherwise>
							<c:if test="${not empty entry.orderWeight}">
								<p>
								<div>
									<span class="labelText"><spring:message code="product.detail.basic.orderweight" /></span>
								</div>
								<div>
							...
							<c:if test="${not empty entry.cubicVolume}">
								<p>
								<div>
									<span class="labelText"><spring:message code="product.detail.basic.cubic" /></span>
								</div>
								<div>
		...
		<td class="text-nowrap"><c:if test="${not isMaterialNumPresent}">
													${entry.quantity} &nbsp; ${entry.product.deliveryUnit} (${entry.product.numerator} &nbsp; ${entry.product.salesUnit})
													</c:if></td>
      ...
      <td>
		...
		<fmt:formatDate pattern="${dateformat}" value="${entry.product.expiryDate}" />
		</td>
		<td>
			<c:choose>
