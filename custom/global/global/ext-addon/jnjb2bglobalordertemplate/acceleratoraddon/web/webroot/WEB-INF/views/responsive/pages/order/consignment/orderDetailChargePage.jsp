...
<%@ taglib prefix="order" tagdir="/WEB-INF/tags/addons/jnjb2bglobalordertemplate/responsive/order/consignment"%>
  ...
  <div class="">
    ...
    <div class="row">
      <div class="col-xs-12 headingTxt content">
        <spring:message code="orderDetailPage.heading" />
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <div class="pull-right btnclsactive marginbtm30">
          <a href="#" id="orderDetailAddToCart1" orderCode="${orderData.code}" class="anchorwhiteText orderDetailAddToCart"><spring:message
              code="orderDetailPage.addOrdertocart" /></a>
        </div>
      </div>
    </div>
              ...
              <tbody>
                <c:forEach items="${entry.scheduleLines}" var="scheduleLine" varStatus="schLineCounter">
                  <order:orderDetailsItem entry="${entry}" isSplitOrder="true" scheduleLine="${scheduleLine}" entryIndex="${entryCounter.count}"
                    schLineIndex="${schLineCounter.count}" isMddSite="${isMddSite}" orderType="${orderData.orderType}"
                    displayDisputeOption="${orderData.displayDisputeOption}" />
                </c:forEach>
              </tbody>
            </c:when>
            <c:otherwise>
              <order:orderDetailsItem entry="${entry}" isSplitOrder="false" isMddSite="${isMddSite}" entryIndex="${entryCounter.count}"
                orderType="${orderData.orderType}" displayDisputeOption="${orderData.displayDisputeOption}" />
            </c:otherwise>
          </c:choose>
        </c:forEach>
      </table>
    ...
    <div class="Subcontainer visible-xs hidden-lg hidden-sm hidden-md">
      <table id="ordersTable" class="table table-bordered table-striped tabsize">
        ...
        <tbody>
          <c:forEach items="${orderData.entries}" var="entry" varStatus="entryCounter">
            <tr>
              <td class="panel-title text-left"><a data-toggle="collapse" data-parent="#accordion" href="#collapse${entryCounter.count}"
                class="ref_no toggle-link panel-collapsed"> <span class="glyphicon glyphicon-plus"></span> ${entry.product.code}
              </a>
                      ...
                      <div>
                        <p>
                          <c:choose>
                            <c:when test="${isMaterialNumPresent}">
                              <b><i><spring:message code="orderDetailPage.orderData.Productmaterial" /><span class="strong">${entry.materialNumber}</span></i></b>
                            </c:when>
                            <c:otherwise>
                            <!-- Fix For AAOL-5695 -->
                              <input type="hidden" value="${entry.product.saleableInd && entry.salesRepDivisionCompatible}" class="saleableInd saleableIndicator"
                                id="${entry.product.code}" qty="${entry.quantity}" />
                              <c:choose>
                        ...
                        <p>
                          <c:if test="${not empty entry.contractNumber}">
                            <p>
                              <span class="smallFont"><spring:message code="orderDetailPage.orderData.contract" /></span> ${entry.contractNumber}
                            </p>
                          </c:if>
                        </p>
                        <p>
                          <c:if test="${not empty entry.hazmatInd}">
                            <div>
                              <p>
                                <spring:message code="orderDetailPage.orderData.HazmatCode" />
                                ${entry.hazmatInd}
                            </div>
                          </c:if>
                        </p>
    ...
    <div class="row basecontainer fee-toggle-container">
      <table class="total-summary-table">
        <tr>
          <c:choose>
            <c:when test="${'ZLZ' eq orderData.orderType}">
              <td class="total-summary-label" style="font-size: 10px !important"><spring:message code="orderDetailPage.orderData.subTotal" /></td>
            </c:when>
            <c:otherwise>
              <td class="total-summary-label" style="font-size: 10px !important"><spring:message code="orderDetailPage.orderData.subTotal" /></td>
              <td class="total-summary-cost totalrps">${orderData.totalNetValue.formattedValue}</td>
            </c:otherwise>
          </c:choose>
        </tr>
        <c:if test="${isMddSite}">
          <c:if test="${orderData.totalFees.value ne '0.0'}">
            <tr>
              <td class="total-summary-label" style="width: 25px;"><spring:message code="orderDetailPage.total.fee" /></td>
              ...
              <td class="toggle-fee"><a data-toggle="collapse" class="toggle-fee-link toggle-link panel-collapsed"
                href="#fee-mobile-collpase${globalCount}"><span class="glyphicon glyphicon-chevron-up"></span></a></td>
            </tr>
            <tr class="fee-panel">
              <td colspan='2'>
                <table id="fee-mobile-collpase${globalCount}" class="fee-collpase-table total-summary-table panel-collapse collapse"
                  ...
                  <c:if test="${orderData.totalDropShipFee.value ne '0.0'}">
                    <tr>
                      <td class="total-summary-label" style="width: 25px; font-size: 10px !important"><spring:message
                          code="orderDetailPage.dropship.fee" /></td>
                  ...
                  <c:if test="${orderData.totalFreightFees.value ne '0.0'}">
                    <tr>
                      <td class="total-summary-label" style="font-size: 10px !important"><spring:message
                          code="orderDetailPage.orderData.freightHandlingFee" /></td>
        ...
        <tr class="summary-bline ">
          <c:choose>
            <c:when test="${'ZLZ' eq orderData.orderType}">
              <td class="total-summary-label"><spring:message code="orderDetailPage.orderData.taxes" /></td>
            </c:when>
            <c:otherwise>
              <td class="total-summary-label"><spring:message code="orderDetailPage.orderData.taxes" /></td>
        ...
        <tr class="total-price-row">
          <td class="total-summary-label"><spring:message code="orderDetailPage.orderData.orderTotal" /></td>
        ...
        <div class="modal-dialog modalcls">
          <div class="modal-content popup">
            ...
            <div class="modal-footer ftrcls">
              ...
              <button type="button" class="btn btnclsactive" data-dismiss="modal" id="accept-btn-addtocart">
      ...
      <div class="modal-content popup">
        ...
        <div class="modal-body" style="font-size: 18px; padding: 25px">
          <div class="panel-info">
            <div class="panel-heading">
              <h4 class="panel-title">
                <span class="glyphicon glyphicon-ok"></span> <span id="dispute-success-info"></span>
              </h4>
            </div>
          </div>
        </div>
