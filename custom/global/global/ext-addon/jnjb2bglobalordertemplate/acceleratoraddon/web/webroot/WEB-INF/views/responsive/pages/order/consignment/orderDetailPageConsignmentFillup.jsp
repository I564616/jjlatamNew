...
<%@ taglib prefix="consignMentFillup" tagdir="/WEB-INF/tags/addons/jnjb2bglobalordertemplate/responsive/order/consignment"%>
...
<input style="display:none;" type="hidden" id= "disputeFlag" value="${disputeFlag}" />
	<div class="">
		...
		<div class="row">
		 ...
		 <consignMentFillup:orderDetailHeaderConsignmentFillup orderData="${orderData}" isMddSite="${isMddSite}" /> 
		<div class="row">
			...
			<table id="ordersTable" class="table table-bordered table-striped sorting-table ">
				<thead>
				  <tr>
					...
					<th class="no-sort" style="width: 120px"><spring:message code="product.description"/></th>
				  ...
				  <c:choose>	
				   <c:when test="false">			
							...
							<c:forEach items="${entry.scheduleLines}" var="scheduleLine" varStatus="schLineCounter">
								<consignMentFillup:orderDetailsItemConsignmentFillup entry="${entry}" isSplitOrder="true" scheduleLine="${scheduleLine}"  entryIndex="${entryCounter.count}" 
					...
					<c:otherwise>
						<consignMentFillup:orderDetailsItemConsignmentFillup entry="${entry}" isSplitOrder="false" isMddSite="${isMddSite}" entryIndex="${entryCounter.count}"  orderType="${orderData.orderType}" 
