...
<div class="">
  <form:form class="searchSortForm" id="searchSortForm1" name="searchSortForm" method="get" action="${refineUrl}" commandName="searchSortForm">
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ordertypedetails">
        ...
        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
          <table class="table">
            <thead>
              <tr>
                <th><strong><spring:message code="orderDetailPage.orderData.orderNumber" /></strong></th>
                <th><strong><spring:message code="orderDetailPage.orderData.orderType" /></strong></th>
                <th><strong><spring:message code="orderDetailPage.orderData.accountNumber" /></strong></th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td class="fontsize25" style="text-align: left !important;">${orderData.orderNumber}</td>
                <td class="fontsize17" style="text-align: left !important;"><c:if test="${not empty orderData.orderType}">
                    <spring:message code="cart.common.orderType.${orderData.orderType}" />
                  </c:if></td>
                <td style="text-align: left !important;"><c:if test="${not empty orderData.soldToAccount}">${orderData.soldToAccount}</c:if></td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12 text-right downloadLinks">
          <p>
            ...
            <span> <input type="submit" class="tertiarybtn pdfdownloadlinks" value="XLS" name="downloadType" /> <span class="pipesymbol">|</span>&nbsp;
              <input type="submit" class="tertiarybtn marginRight pdfdownloadlinks" value="PDF" name="downloadType" /></span>
          </p>
        </div>
      </div>
      <div class="row col-lg-12 col-md-12 col-sm-12 col-xs-12 shippingAddressDetails">
        ...
        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12 orderDetailsAndStatus" id="order-detail-right-panel">
          <div id="orderDetail-rightside">
            <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12 padding30px">
              <c:if test="${not empty orderData.purchaseOrderNumber}">
                <h6><spring:message code="order.customer.po" /></h6>
                <p>${orderData.purchaseOrderNumber}</p>
              </c:if>
              <h6><spring:message code="order.po.date" /></h6>
              ...
              <h6 class="text-uppercase"><spring:message code="stock.user" /></h6>
              <p>${orderData.stockUser}</p>
            </div>
            <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12 padding30px">
              ...
              <c:if test="${isMddSite}">
                ...
                <h6><spring:message code="order.overall.status" /></h6>
                ...
                <h6><spring:message code="order.end.user" /></h6>
                <p>${orderData.endUser}</p>
                <c:if test="${not empty orderData.paymentInfo}">
                  <div>
                    ...
                    <div>
                      <span>${orderData.paymentInfo.cardType}</span></br> <span>${orderData.paymentInfo.cardNumber}</span></br> <span>${orderData.paymentInfo.expiryMonth}/${orderData.paymentInfo.expiryYear}</span>
                    </div>
                  </div>
                </c:if>
              </c:if>
          ...
          <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
            ...
            <a class="tertiarybtn disputeOrder" id="disputeOrder" href="${disputedetailsUrl}"><spring:message
                code="orderDetailPage.orderData.disputeThisOrder" /></a>
            <!-- AAOL-3681 -->
            <c:if test="${orderData.displayDisputeOption}">
              <span> <a
                class="tertiarybtn disputeOrder" id="disputeOrder" href="${disputedetailsUrl}"> <spring:message
                    code="orderDetailPage.orderData.disputeThisOrder" />
              </a>
              </span>
            </c:if>
            <!-- AAOL-3681 -->
...
<div class="last">
              ...
              <c:forEach items="${shippingTrackingInfo.value}" var="data">
                <c:choose>
                  <c:when test="${data.trackingUrl eq '#' && !isMddSite}">
                    <p class="textBlack">${data.trackingId}</p>
                  </c:when>
                  <c:otherwise>
                    <p>
  ...
  <c:if test="${orderData.rgaLinkURL}">
    ...
    <div>
      <a id="rgaId" href="${rgaUrl}"><spring:message code="orderDetailPage.orderData.rgaLink" /></a>
    </div>
  </c:if>
  <c:if test="${orderData.codLinkURL}">
    ...
    <div>
      <a id="cmodId" href="${cmodUrl}"><spring:message code="orderDetailPage.orderData.codLink" /></a>
    </div>
  </c:if>
</div>
<!--  Shipping Information  pop up-->
<div class="modal fade jnj-popup" id="shipping-detail-popup" role="dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row" id="shipping-detail-panel">
          <c:if test="${isMddSite}">
            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 shipping-detail-row">
              ...
              <div>
                <c:choose>
                  ...
                  <c:otherwise>
                    <p>
                      <b><spring:message code="orderDetailPage.orderData.notAvailable" /></b>
                    </p>
                  </c:otherwise>
                </c:choose>
              </div>
