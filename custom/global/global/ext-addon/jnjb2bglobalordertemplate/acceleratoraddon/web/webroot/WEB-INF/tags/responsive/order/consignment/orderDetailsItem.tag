...
<tr>
  ...
  <%-- td class="orderHistoryList">${entry.sapOrderlineNumber}
			<c:if test="${entry.expandableSchedules}">
				<div class="expendOrderDetailArrowHistory rightArrow">+</div>
			</c:if>
		</td> --%>
  <td class="paddingtop10px">${entry.entryNumber+1}</td>
  <!-- Adding for AAOL-3609 -->
  <td class="paddingtop10px">
    <div class="column2 display-row">
      <c:choose>
      ...
      <div class="orderProdDesc right table-cell">
        <c:choose>
          ...
          <c:otherwise>
            ...
            <div>
              <spring:message code="orderDetailPage.orderData.gtin" />${entry.product.gtin}</div>
            <a><p>
                <c:url
                  value="/order-history/order/disputeItem?orderCode=${orderData.code}&productCode=${entry.product.baseMaterialNumber} 
								&contractNum=${entry.contractNumber}&totalPrice=${entry.totalPrice.value}"
                  var="disputeItemUrl" />
                <a href="${disputeItemUrl}" class="smallFont disputeItem" orderCode="${orderData.code}"
                  productCode="${entry.product.baseMaterialNumber}" contractNum="${entry.contractNumber}" totalPrice="${entry.totalPrice.value}">
                  <spring:message code="orderDetailPage.orderData.disputeThisItem" />
                </a>
              </p></a>
            </p>
          </c:otherwise>
        ...
        <c:choose>
          <c:when test="${isMddSite}">
            ...
            <c:if test="${not empty entry.product.gtin}">
              <p>
                <span class="smallFont"> <spring:message code="orderDetailPage.orderData.gtin" /></span> ${entry.product.gtin}
              </p>
            </c:if>
            ...
            <c:if test="${not empty entry.lotNumber}">
              <p>
                <span class="smallFont">Lot Number:</span></br> <span class="txtFont">${entry.lotNumber}</span>
              </p>
            </c:if>
          ...
          <c:otherwise>
            <p>
              <span class="smallFont"><spring:message code="product.detail.basic.upc" /></span> ${entry.product.upc}
            </p>
          </c:otherwise>
        </c:choose>
        <%-- 	<c:choose>
					<c:when test="${isMddSite}">
						...
						<c:if
							test="${not empty entry.expectedShipDate and not entry.expandableSchedules}">
							<p>
								<span class="labelText">Estimated Ship Date:</span></br> <span
									class="txtFont"><fmt:formatDate
										value="${entry.expectedShipDate}" pattern="MM/dd/yyyy " /></span>
							</p>
						</c:if>
  ...
  <!-- 	AAOL-3638 End -->
  <td class="total-price-cell"><c:choose>
