...
<div class="row jnj-panel mainbody-container backOrderReportBlock">
...
<c:url value="/reports/inventory" var="inventoryURL" />
 <c:url value="/reports/inventoryAnalysis/cutorder" var="inventoryCutReportUrl"></c:url>
  <input type="hidden" id="originalFormAction" value="${backOrderURL}" /> <input type="hidden" id="accountid" value="${currentAccountId}" />
          ...
          <c:when test="${inventoryAnalysis eq true}">
            ...
            <div class="form-group col-lg-5 col-md-5 col-sm-6 col-xs-12">
              <label class="pull-left form-label form-label-select-large boldtext"><spring:message code='reports.report.type' /></label> <select
                ...
                <option selected="selected" value="${inventoryBackOrderURL}"><spring:message code='reports.backorder.header' /></option>
                <option value="${inventoryCutReportUrl}"><spring:message code='cutReport.label' /></option>
                <c:if test="${inventry eq 'true'}">
          ...
          <c:otherwise>
            <div class="form-group col-lg-5 col-md-5 col-sm-6 col-xs-12">
              <label class="pull-left form-label form-label-select-large boldtext"><spring:message code='reports.report.type' /></label> <select
                ...
                <option selected="selected" value="${backOrderURL}"><spring:message code='reports.backorder.header' /></option>
          ...
          <thead>
            <tr>
              ...
              <%-- <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.orderDate' /></th>
						...
						<th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.shipTo' /></th>
						...
						<th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.itemPriceRpt' /></th>
						<th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.total.price' /></th> --%>
              <th class="text-left-cl text-uppercase"><spring:message code='orderHistoryPage.account' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.customerPORef' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.jnjorderNumber' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.orderDate' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.franchise' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.productName' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.productCodeRpt' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.qtyRpt' /></th>
              <th class="text-left-cl text-uppercase"><spring:message code='reports.backorder.table.unitRpt' /></th>
          ...
          <tbody>
            ...
            <c:forEach items="${jnjGTBackorderReportResponseDataList}" var="backorderLine" varStatus="count">
              ...
              <tr>
                ...
                <td class="text-left-cl">${backorderLine.orderDate}</td>
                <td class="text-left-cl">${backorderLine.operatingCompany}</td>
                ...
                <td class="text-left-cl"><span class="productCode"> <c:url value="${backorderLine.productUrl}" var="productURL" /> <a
                    href="${productURL}">${backorderLine.productCode}</a>
						...
						<c:if test="${backorderLine.qty ne 'null'}">
						${backorderLine.qty}
						</c:if>
						</td>
                <td class="text-left-cl">${backorderLine.unit}</td>
                <td class="text-left-cl">${backorderLine.status}</td>
                <td class="text-left-cl">${backorderLine.estimatedAvailability}</td>
                <%-- <td class="text-left-cl">${backorderLine.currency}${backorderLine.itemPrice}</td>
						 <c:set var="total" value="${total+itemExtendedPrice}"/>
						<td class="text-left-cl">
						<input type="hidden" value="${backorderLine.extendedPrice}" class="extendedPriceField" />
						<span class="priceList">${backorderLine.currency}${backorderLine.extendedPrice}</span>
						</td> --%>
              </tr>
            </c:forEach>
          </tbody>
                ...
                <td><a data-toggle="collapse" data-parent="#accordion" href="#report-mobi-${Count.count}"
                  ...
                  <div id="report-mobi-${Count.count}" class="panel-collapse collapse">
                    <div class="panel-body details">
                      ...
                      <div>${backorderLine.orderDate}</div>
                      <div class="lblFont boldtext">
