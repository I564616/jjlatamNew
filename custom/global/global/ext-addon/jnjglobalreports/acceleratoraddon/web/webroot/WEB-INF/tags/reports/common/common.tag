		...
		<div class="row jnj-panel-body">
			<div class="form-group col-lg-4 col-md-4 col-sm-4 col-xs-12">
				...
				<select class="form-control form-element form-element-select" id="reportCategory">
					...
					<!--changes for 2422 to hide the financial option -->
						...
						<!-- end of changes for 2422 to hide the financial option -->
					</c:forEach>													
				</select>
			</div>
			<div class="form-group col-lg-4 col-md-4 col-sm-4 col-xs-12 col-xs-12">
				...
				<select class="form-control form-element form-element-select" id="reportTypes">
								...
								<c:if test="${reportType.key eq 'boReport' || reportType.key eq 'inventoryReport' || 
									...
									<c:choose>
										<c:when test="${subReportUrl eq reportType.key}">
											<option value="${reportType.value}" selected="selected"><spring:message code='${reportType.key}.reportType'/></option>							
										</c:when>
										<c:otherwise>
											<option value="${reportType.value}"><spring:message code='${reportType.key}.reportType'/></option>	
										</c:otherwise>
									</c:choose>
								</c:if>
							</c:when>
					...
					<c:if test="${siteName eq 'CONSUMER'}">
						<c:if test="${reportType.key eq 'cutReport' || reportType.key eq 'singleProductReport' || reportType.key eq 'multiProductReport' }">
							<c:choose>
								<c:when test="${subReportUrl eq reportType.key}">
									<option value="${reportType.value}" selected="selected"><spring:message code='${reportType.key}.reportType'/></option>							
								</c:when>
			...
			<c:if test="${showCurrency eq true}">
				<div class="form-group col-lg-4 col-md-4 col-sm-4 col-xs-12 col-xs-12">
						<label class="form-label currency-label-select-large">Currency</label>	
