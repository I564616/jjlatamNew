...
<%@ attribute name="varRowCount" required="true" type="java.lang.Integer"%>
 <!-- consignmentFillUp/multiValidateCartsEntries.tag -->
...
<div class="hidden-xs dropShipmentTable"	id="dropShipmentTable-${varRowCount}">
	...
	<table id="datatab-desktop"
		class="table table-bordered table-striped sorting-table fee-price-table">
		<thead>
			<tr>
		...
		<tbody>
			<c:forEach items="${jnjCartData.entries}" var="entry"  varStatus="count">
				<tr id="orderentry-${count.count}">
					...
					<td class="text-left">
						<consignmentFillUpCart:productDescriptionBeforeValidation entry="${entry}" showRemoveLink="true" showStatus="false" /></td>
	...
	<consignmentFillUpCart:multiValidateCartTotalItem jnjCartData="${jnjCartData}" varRowCount="${varRowCount}"/>
