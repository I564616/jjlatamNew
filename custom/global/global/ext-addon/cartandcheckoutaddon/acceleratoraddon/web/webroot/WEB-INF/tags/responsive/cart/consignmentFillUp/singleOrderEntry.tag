...
<!-- consignmentFillUp/singleOrderEntry.tag -->
<div class="mainbody-container">
  <div class="hidden-xs">
    <table id="datatab-desktop" class="table table-bordered table-striped sorting-table fee-price-table">
      <thead>
        <tr>
          <th style="width: 60px"><spring:message code="cart.review.entry.number" /></th>
          <th class="no-sort text-left text-uppercase"><spring:message code="product.description" /></th>
          <%-- <th class="no-sort text-left text-uppercase"><spring:message code="cart.validate.shippingMethod" /></th> --%>
          <th class="no-sort text-left text-uppercase"><spring:message code="cart.validate.quantity" /></th>
          <%-- <th class="no-sort text-uppercase"><spring:message code="product.search.uom" /></th> --%>
      ...
      <tbody>
        <c:forEach items="${orderData.entries}" var="entry" varStatus="count">
          <tr id="orderentry-${count.count}">
            <td>${count.count}</td>
            <td class="text-left"><consignmentFillUpCart:ProductDescriptionOrderComplete entry="${entry}" showRemoveLink="true" showStatus="false" /></td>
            <%-- <td>${entry.shippingMethod}<br> <c:if test="${not entry.expandableSchedules}">
                <commonTags:entryLevelDates entry="${entry}" />
              </c:if>
            </td> --%>
            <td>
              <div>${entry.quantity}</div> <p class="thirdline">
          ...
          <c:if test="${freeGoodsMap ne null}">
            ...
            <c:if test="${not empty valueObject.itemCategory}">
              <tr class="noborder">
                <td class="text-right"><consignmentFillUpCart:productDescriptionBeforeValidation entry="${entry}" showRemoveLink="false"
                    showStatus="false" /></td>
      ...
      <table id="datatab-mobile" class="table table-bordered table-striped sorting-table">
        ...
        <tbody>
          <c:forEach items="${orderData.entries}" var="entry" varStatus="scount">
            <tr>
              <td class="text-left"><consignmentFillUpCart:ProductDescriptionOrderComplete rowcount="${scount.count}" entry="${entry}"
