...
<!-- consignmentCharge/validateEntries.tag -->
<div id="AddItemsCartpage">
  ...
  <div class="mainbody-container" id="consignmentOrderPage">
    ...
    <consignmentChargeCart:validatePageHeader currentPage="cartValidationPage" />
    <c:choose>
      <c:when test="${splitCart}">
        <c:forEach items="${jnjCartDataList}" var="jnjCartData" varStatus="count">
          <consignmentChargeCart:multiValidateCartsEntries jnjCartData="${jnjCartData}" varRowCount="${count.count}" />
        </c:forEach>
        <consignmentChargeCart:cartTotals />
      </c:when>
      <c:otherwise>
        <consignmentChargeCart:singleValidateCartEntry />
      </c:otherwise>
    </c:choose>
  </div>
  ...
  <div class="row validatebtn">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      ...
      <div class="float-right-to-none">
        ...
        <button type="button" class="btn btnclsactive placeorder placeOrderBtnConsignment">
          <spring:message code="cart.common.placeOrder" />
        </button>
    ...
    <div class="modal-dialog modalcls">
      <div class="modal-content popup">
        ...
        <div class="modal-footer ftrcls">
          ...
          <button type="button" class="btn btnclsactive" data-dismiss="modal" id="accept-btn-placeorder">
