...
<%@ attribute name="jnjOrderData" required="true"
...
<c:url value="/order-history/order/${jnjorderData.orderNumber}"
	var="orderDetailURL" />
	<!-- replenish/multiOrderEntries.tag -->
<div class="mainbody-container">
	<div class="hidden-xs dropShipmentTable"
		id="dropShipmentTable-${varRowCount}">
		<div class="dropshipment-label-value">
			<label class="dropshipment-label"> <strong><spring:message
				...
				<div class="dropshipment-value">
					<spring:message
						code="cart.common.orderType.${jnjOrderData.orderType}" />
				</div>
			</label> <label class="dropshipment-label"> <strong><spring:message
						code="cart.confirmation.orderNumber" /></strong>
				<div class="dropshipment-value">
		...
		class="table table-bordered table-striped sorting-table-lines fee-price-table">
			<thead>
				<tr>
					...
					<th class="no-sort text-left text-uppercase"><spring:message
							code="cart.validate.shippingMethod" /></th>
			...
			<tbody>
				<c:forEach items="${jnjOrderData.entries}" var="entry"
					...
					<tr id="orderentry-${count.count}">
						<td class="text-left"><standardCart:ProductDescriptionOrderComplete
								entry="${entry}" showRemoveLink="true" showStatus="false" /></td>
						<td>${entry.shippingMethod}<br> <c:if
						...
						<td class="valign-middle totalrps fee-cell"><ycommerce:testId
					...
					<td colspan='2'>
						<table id="fee-mobile-collpase${varRowCount}"
							...
							<tr>
								...
								<td class="total-summary-cost totalrps" style="font-size:12px !important"><format:price priceData="${jnjOrderData.totalDropShipFee}" /></td>
							</tr>
							<tr>
								...
								<td class="total-summary-cost totalrps" style="font-size:12px !important"><format:price priceData="${jnjOrderData.totalminimumOrderFee}" /></td>
							</tr>
							
							<tr>
								<td class="total-summary-label" style="font-size:10px !important"><spring:message code="cart.validate.cartTotal.freightCost"/></td>
								<td class="total-summary-cost totalrps" style="font-size:12px !important"><format:price priceData="${jnjOrderData.totalFreightFees}"/></td>
							</tr>
						</table>
					</td>
				</tr>
