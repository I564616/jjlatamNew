...
<!--Order Derail Row starts-->
	         	...
	         	<c:choose>
					<c:when test="${splitCart}">
						<c:forEach items="${jnjCartDataList}" var="jnjCartData">
							<international:multiValidateCartsEntries jnjCartData="${jnjCartData}" varRowCount="${count.count}"  />
						</c:forEach>
						<div class="sectionBlock buttonWrapperWithBG borDer">
							<div class="txtRight">
								<international:cartTotals/>
							</div>
						</div>
					</c:when>
					<c:otherwise>
						<international:singleValidateCartEntry/> 
						<international:cartTotals/>
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<div class="float-right-to-none">
				<%-- <button type="button" class="btn btnclsnormal templatebtn">
