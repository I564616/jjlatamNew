...
<!-- replenish/singleOrderEntry.tag -->
		...
		<table id="datatab-desktop" 	class="table table-bordered table-striped sorting-table-lines fee-price-table">
			<thead>
				<tr>
					...
					<th class="no-sort text-left text-uppercase">
						<spring:message code="cart.validate.shippingMethod" /></th>
					<th class="no-sort text-uppercase">
						<spring:message code="cart.validate.quantity" /></th>
					<th class="no-sort unitprice-thead">
						<spring:message code="cart.validate.unitPrice" /></th>
					<th class="no-sort total-thead fee-cell multitotal-thead text-uppercase">
						<spring:message code="cart.validate.total" /></th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${orderData.entries}" var="entry" varStatus="count">
					<tr id="orderentry-${count.count}">
						...
						<td class="text-left">
							<standardCart:ProductDescriptionOrderComplete entry="${entry}" showRemoveLink="true" showStatus="false" /></td>
			...
			<table id="datatab-mobile" class="table table-bordered table-striped sorting-table-lines">
				...
				<tbody>
					<c:forEach items="${orderData.entries}" var="entry"
						...
						<tr>
							<td class="text-left">
								<standardCart:ProductDescriptionOrderComplete rowcount="${scount.count}" entry="${entry}" showRemoveLink="true" showStatus="false" />
	...
	<table class="total-summary-table">
		<tr>
			...
			<td class="total-summary-cost totalrps">
				<format:price priceData="${orderData.subTotal}" /></td>
		</tr>
		<tr>
			<td class="total-summary-label">
				<spring:message code="cart.validate.cartTotal.fees" /></td>
			<td class="total-summary-cost totalrps">
				<format:price priceData="${orderData.totalFees}" /></td>
			<td class="toggle-fee">
				<a data-toggle="collapse" 	class="toggle-fee-link toggle-link panel-collapsed"
				href="#fee-mobile-collpase">
