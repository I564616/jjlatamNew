...
<!-- consignmentFillUp/orderEntries.tag -->
<c:choose>
  <c:when test="${splitOrder}">
    <c:forEach items="${orderDataList}" var="jnjorderData" varStatus="count">
      <consignmentFillUpCart:multiOrderEntries jnjOrderData="${jnjorderData}" varRowCount="${count.count}" />
    </c:forEach>
  </c:when>
  <c:otherwise>
    <consignmentFillUpCart:singleOrderEntry />
  </c:otherwise>
</c:choose>