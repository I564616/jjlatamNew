...
<!-- consignmentFillUp/cartEntries.tag -->
    ...
    <div class="col-lg-6 col-md-6 col-sm-7 col-xs-12">
      ...
      <div class="orderType-holder">
        ...
        <c:choose>
          ...
          <c:otherwise>
            <button type="button" form="consignmentFillupForm" class="btn btnclsactive consignmentFillupCartValidate" id="consignmentFillupCartValidate"><spring:message code="cart.review.progressBar.validate"/></button>
          </c:otherwise>
        </c:choose>
        <!-- No charge Order AAOL-3392 -->
      </div>
...
<div class="row jnjPanelbg">
  ...
  <consignmentFillUpCart:cartPageHeader currentPage="cartPage"/>
      ...
      <tbody id="AddItemsCartpage" >
        ...
        <!-- Soumitra - Added for showing SAP errors received upon validation from validateorder method AAOL-4357 -->
                      <c:if test="${not empty sapErrorResponse}">
                        <div style="color: red;">
                          ...
                          <c:set var="exceededByQuantity" value=""></c:set>
                          <c:forEach var="sapErrorforProduct" varStatus="countProductError"
                            ...
                            <c:if test="${sapErrorforProduct.id eq 'CBC_QTY_EXCEED'}">
                            <c:set var="exceededByQuantity" value="${sapErrorforProduct.messagev2}"></c:set>
                            </c:if>
                          </c:forEach>
                        </div>
                      </c:if>
        ...
        <td class="vertical-align-top text-center">${count.count}</td>
          <td class="text-left" width="40%">
            <consignmentFillUpCart:productDescriptionBeforeValidation entry="${cartData.entries[fieldLength - count.count]}" errorCode="${validationErrorMsg}" showRemoveLink="true"  showStatus="false"  rowcount="${count.count}"/>
          </td>
          <td width="40%">
            <div class="cart-update-link">  <a href="javascript:void();" id="quantity${cartData.entries[fieldLength - count.count].entryNumber}_update" entryNum="${cartData.entries[fieldLength - count.count].entryNumber}" class="qtyUpdateTextBox1" ><spring:message code="cart.review.productDesc.updateItem"/></a></div> 
              ...
              </form:form><c:if test="${fn:contains(errorString, 'CBC_QTY_EXCEED')}">
                              <div class="error">
                                 <spring:message code="cbc.quantity.exceed"/> ${exceededByQuantity} <spring:message code="cbc.quantity.exceedByUnit"/>
                              </div>
                             </c:if>
            </td> 
          <!-- <td></td>
...
<%--  <div class="row basecontainer">
 ...
 <consignmentFillUpCart:cartPageActions/> 
  ...
  <div class="modal-dialog modalcls modal-md" id="validateOrderPOpopup">
    <div class="modal-content">
      ...
      <c:url value="/cart/validate" var="orderValidateUrl" />
        <a href="#"  class="pull-left"  data-dismiss="modal"><spring:message code="account.buildorder.cancel"/></a>
      <button type="button" class="btn btnclsactive pull-right" data-dismiss="modal" onclick="location.href='${orderValidateUrl}'"><spring:message code="cart.review.cartPageAction.validateorder"/></button>
      </div>  
    </div>
  </div>
</div>
