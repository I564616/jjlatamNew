...
<%@ taglib prefix="checkout" tagdir="/WEB-INF/tags/responsive/checkout/single" %>
...
<spring:url value="/_ui/responsive/common/images/spinner.gif" var="spinnerUrl" />
...
<div class="checkout_summary_flow_b ${paymentInfoOk ? 'complete' : ''}" id="checkout_summary_payment_div">
	...
	<ycommerce:testId code="checkout_changePayment_element">
		<c:choose>
			...
			<c:otherwise>
				<button class="form change_payment_method_button" data-payment_id="${not empty paymentInfo ? paymentInfo.id : ''}"><spring:theme code="checkout.summary.paymentMethod.editPaymentMethod"/></button>
			</c:otherwise>
		</c:choose>
	</ycommerce:testId>
</div>
