...
<div class="row jnjPanelbg">
	...
	<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 surgeon-name-holder">
		<%-- <c:if test="${not empty cartData.dropShipAccount}"> --%>
			<div class="txt-label-inline"><spring:message code="cart.deliver.shipToAcc"/></div>
		<%-- </c:if> --%>
		...
		<div class="txt-box-inline">
			<c:if test="${currentPage eq 'cartPage'}">
				<button class="drop-ship-account-list-icon fa fa-list changeShipAddLighboxLink ${hideSelectDropShip}" 
						data-toggle="modal" data-url="${dropShipURL}" id="selectDropShipLink"></button> <!-- data-target="#dropship-account-popup" -->
			</c:if>
		</div>
		<div class="registerError"></div>
	</div>	
	<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
		<%-- <c:if test="${not empty cartData.attention}"> --%>
			<div class="txt-label-inline no-charge-reason"><spring:message code="cart.common.attention"/></div>
		<%-- </c:if> --%>
		<div class="txt-box-inline no-charge-select">
		 	<span>${cartData.attention}</span>
			<%-- <input type="text" id="attention" name="attention"  class="form-control ${addOnVisibilityCss}"  ${disabled} value="${cartData.attention}"> --%>
		</div>	
	</div>	
</div>
	<%--
