      ...
      <tr>
        ...
        <%-- 						code="cart.validate.total" /></th> --%>
				<th class="no-sort unitprice-thead"><spring:message
						code="cart.validate.unitPrice" /></th>
				<th class="no-sort text-center fee-cell multitotal-thead  text-uppercase"><spring:message
						code="cart.validate.total" /></th>
				<th class="no-sort total-thead fee-cell text-uppercase"><spring:message 
						code="cart.review.productDesc.status" /></th>		
      </tr>
    </thead>
    <tbody>
      ...
      <c:forEach items="${entry.scheduleLines}" var="deliverySch" varStatus="deliverySchCount">
        <tr id="orderentry-${count.count}">
          <td class="snoData valign-middle">${count.count}.${deliverySchCount.count}</td>
          <td class="text-left valign-middle"><standardCart:productDescriptionBeforeValidation entry="${entry}" showRemoveLink="true" showStatus="false" /></td>
