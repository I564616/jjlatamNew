...
<template:page pageTitle="${pageTitle}">
	<div id="ordercompletePage" class="orderReviewpage">
			...
			<div class="col-lg-12 col-md-12">
					...
					<div class="col-lg-5 col-md-5">
						<div class="btn-group pull-right">
							<ul id="breadcrumbs-one">
								<li><a href="shipping"><spring:message
											code="cart.review.tabsShipping" /></a></li>
								<li><a href="paymentContinue"><spring:message
											code="cart.review.tabsPayment" /></a></li>
								<li><a href="orderReview"><strong><spring:message
												code="cart.review.tabsReview" /></strong></a></li>
							</ul>
						</div>
					</div>
				</div>
				<div class="row headingTxt content">
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 orderreviewtxt">
						<spring:message code="cart.review.review" />
					</div>
		...
		<div class="row table-padding">
			...
			<c:if test="${not empty cartData.purchaseOrderNumber}">
				...
				<div class="order-column50">
					<c:if test="${divisionData.isSpine}">
						<p class="subhead boldtext text-uppercase">
							<spring:message code="cart.common.spinSalesRepUCN"></spring:message>
						</P>
						<div class="subtext boldtext">${cartData.spineSalesRepUCN}</div>
					</c:if>
					<c:if test="${divisionData.isMitek}">
						<p class="subhead boldtext text-uppercase">
							<spring:message code="cart.review.entry.specialSP"></spring:message>
						</P>
						<div class="subtext boldtext">${cartData.spineSalesRepUCN}</div>
					</c:if>

				</div>
				<c:if test="${not empty cartData.surgeonName}">
					<div class="order-column50">
					
						<p class="subhead boldtext text-uppercase">
							<spring:message code="cart.common.surgeonName"></spring:message>
						</P>
						<div class="subtext boldtext">${cartData.surgeonName}</div>
						<div>
							<a type="button" href="" data-toggle="modal"
								id="showSurgeryInfoPopupReview"><spring:message
									code="cart.deliver.surgeryInfo" /></a>
						</div>
						<div class="visible-sm visible-xs">
							<a class="captureSignature" href="javascript:;"><spring:message
									code="orderreviw.CaptureSignature" /></a>
						</div>
					</div>
				</c:if>
				<div class="order-column-x-100">
				...
				<c:if test="${not empty cartData.dropShipAccount}">
					...
					<div class="order-column50">
						...
						<div class="subtext boldtext">${cartData.distributorPONumber}</div>
					</div>
					<div class="order-column-x-100">
						<hr>
					</div>
				...
				<div class="order-column50">
					<p class="subhead boldtext text-uppercase"><spring:message code ="checkout.summary.paymentMethod.header"/></P>
					<div class="subtext boldtext">${cartData.paymentType.displayName}</div>
					<div>
						<a target="_blank" href="${cartData.attachDocURL}"><spring:message
					code="cart.deliver.deliverOrderForm" /></a>
					</div>
				</div>				
				<div class="order-column-x-100">
					<hr>
				...
				<cart:deliveryAddress deliveryAddress="${cartData.deliveryAddress}"
					companyName="${cartData.b2bUnitName}" />
			...
			<div
				class="col-lg-5 col-md-5 col-sm-5 col-xs-12 placeorder-btn-holder">
				<button type="button"
					class="btn btnclsactive pull-right margintop12px placeOrderBtn">
		...
		<br>
		<br>
		<deliveredCart:viewSurgeryInfoDiv />
		<deliveredCart:reviewEntries />
		<div class="row bottomrightbuttons">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="pull-right">
					<button type="button" class="btn btnclsnormal templatebtn saveorderastemplate">
						<spring:message code="cart.common.saveastemplate"/></button>
					<button type="button"
						class="btn btnclsactive placeorder placeOrderBtn">
						<spring:message code="cart.common.placeOrder" />
					</button>
				</div>
			</div>
...
<div id="surgeryInfoPopupHolderRev"></div>
