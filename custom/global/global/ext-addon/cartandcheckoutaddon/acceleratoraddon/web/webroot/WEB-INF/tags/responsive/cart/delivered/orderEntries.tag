...
<c:choose>
	<c:when test="${splitOrder}">
		<c:forEach items="${orderDataList}" var="jnjorderData"
			varStatus="count">
			<deliveredCart:multiOrderEntries jnjOrderData="${jnjorderData}"
				varRowCount="${count.count}" />
		</c:forEach>
	</c:when>
	<c:otherwise>
		<deliveredCart:singleOrderEntry />
	</c:otherwise>
</c:choose>