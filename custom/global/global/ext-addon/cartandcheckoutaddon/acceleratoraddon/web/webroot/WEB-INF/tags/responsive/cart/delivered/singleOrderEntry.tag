						...
						<td>
							...
							<div class="text-left">
								<div class="tablecell-content">
									<div class="nowrap-txt cell-label">
										<spring:message code="cart.review.entry.specialSP"></spring:message>
									</div>
									<div class="nowrap-txt cell-value">${entry.specialStockPartner}</div>
								</div>
								<c:choose>
								<c:when test="${not empty  entry.lotNumber}">
									<div class="tablecell-content">
										<div class="nowrap-txt cell-label">
											<spring:message code="cart.review.entry.lotNumber"></spring:message>
										</div>
										<div class="nowrap-txt cell-value">${entry.lotNumber}</div>
									</div>
			...
			<table id="datatab-mobile"
				...
				<tbody>
					<c:forEach items="${orderData.entries}" var="entry"
						...
						<tr>
							<td class="text-left"><standardCart:ProductDescriptionOrderComplete
									rowcount="${scount.count}" entry="${entry}"
									showRemoveLink="true" showStatus="false" />

								<div id="mobi-collapse${scount.count}"
	...
	<div class="row basecontainer fee-toggle-container">
		<table class="total-summary-table">
					...
					<table id="fee-mobile-collpase"
						...
						<tr>
							<td class="total-summary-label"
								style="width: 25px; font-size: 10px !important"><spring:message
									code="cart.validate.cartTotal.dropShipFee" /></td>
							<td class="total-summary-cost totalrps"
								style="font-size: 12px !important"><format:price
									priceData="${orderData.totalDropShipFee}" /></td>
						</tr>
						<tr>
							<td class="total-summary-label"
								style="font-size: 10px !important"><spring:message
									code="cart.validate.cartTotal.minimumOrderFee" /></td>
							<td class="total-summary-cost totalrps"
								style="font-size: 12px !important"><format:price
						...
						<tr>
							<td class="total-summary-label"
								style="font-size: 10px !important"><spring:message
									code="cart.validate.cartTotal.freightCost" /></td>
							<td class="total-summary-cost totalrps"
								style="font-size: 12px !important"><format:price
									priceData="${orderData.totalFreightFees}" /></td>
						</tr>
					</table>
				</td>
			</tr>
			...
			<tr class="summary-bline ">
				...
				<td class="total-summary-cost totalrps"><format:price
						priceData="${orderData.totalTax}" /></td>
			</tr>
