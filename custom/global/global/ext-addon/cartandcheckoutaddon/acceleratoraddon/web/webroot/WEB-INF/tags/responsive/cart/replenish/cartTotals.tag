...
<div class="row basecontainer fee-toggle-container">
	<table class="total-summary-table">
		...
		<tr>
			...
			<td class="total-summary-cost totalrps">
				<format:price priceData="${cartData.totalFees}" />
			</td>
			<td class="toggle-fee">
				<a data-toggle="collapse" class="toggle-fee-link toggle-link panel-collapsed" href="#fee-mobile-collpase-total" >
...
<p>
	...
	<span class="jnjID"><format:price priceData="${cartData.subTotal}" /></span>
</p>		
	...
	<div>					
		<c:if test="${cartData.totalDropShipFee.value ne '0.0'}">
			<p class="freightTxt">	
				...
				<span class="jnjID"><format:price priceData="${cartData.totalDropShipFee}" /></span></p>
		</c:if>					
		<c:if test="${cartData.totalminimumOrderFee.value ne '0.0'}">
			<p class="freightTxt">
				...
				<span class="jnjID"><format:price priceData="${cartData.totalminimumOrderFee}" /></span>
		</c:if>
		<c:if test="${cartData.totalFreightFees.value ne '0.0'}">
			<p class="freightTxt">
				...
				<span class="jnjID"> <format:price priceData="${cartData.totalFreightFees}"/>
				</span>
			</p>
		</c:if>	
	</div>
...
<c:if test="${cartData.hsaPromotion.value ne '0.0'}">
	<p>
		...
		<span class="jnjID"><format:price priceData="${cartData.hsaPromotion}"/>
		</span>
	</p>
</c:if>
<p>
