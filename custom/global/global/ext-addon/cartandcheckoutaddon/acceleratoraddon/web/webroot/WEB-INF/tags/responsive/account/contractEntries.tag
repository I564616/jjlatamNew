...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<input  type="button" class="btn btnclsactive pull-right addtoCart contract-add-to-cart primarybtn addContractToCart" 
				value= '<label:message messageCode="contract.detail.button.addtocart" />'/>
	</div>
</div>

<form:form id="addContractToCartForm" action="${contractFormAction}" method="post" commandName="contractForm">
					...
					<tr>
						<th class="no-sort text-uppercase">
							<div class="checkbox checkbox-info selectchkbox">
								...
								<label for="contract-select-all"  id="contract-head-chck-label"><label:message messageCode="search.result.selectAll"/></label>
							</div>
						</th>
						<th class="no-sort text-uppercase"><label:message messageCode="contract.itemPrice"/></th>
						<c:if test="${documentType == 'ZCQ'}">	
							<th class="no-sort text-uppercase contract-cnu-head"><label:message messageCode="contract.unitOfMeasure"/></th>
							<th class="no-sort text-uppercase"><label:message messageCode="contract.contractQty"/></th>
							<th class="no-sort text-uppercase"><label:message messageCode="contract.consumed"/></th>
							<th class="no-sort text-uppercase"><label:message messageCode="contract.balance"/></th>
						</c:if>
						<c:if test="${documentType == 'ZCV'}">	
							<th class="no-sort text-uppercase"><span><label:message messageCode="contract.totalValue"/></th>
							<th class="no-sort text-uppercase"><span><label:message messageCode="contract.consumedValue"/></th>
							<th class="no-sort text-uppercase"><label:message messageCode="contract.contractBalance"/></th>
						</c:if>
					</tr>
				</thead>
				<tbody>
						...
						<td class="column0">
							<div class="display-table-row">
								...
								<div class="contract-check-content display-table-cell">
									<a href="${productUrl}"><label:message messageCode="product.jnj.id"/>#&nbsp;${contractEntry.productCode}</a> 
								...
								<td class="column2">
									<c:choose>
										<c:when test="${not empty entry.value.totalAmount}">
											<span><format:price priceData="${entry.value.totalAmount}" hideZeroValue="true"/>&nbsp;</span>
										</c:when>
										<c:otherwise>
											<span>&nbsp;</span>
										</c:otherwise>
							...
							<td class="column3">
								<c:choose>
										<c:when test="${not empty entry.value.consumedAmount}">
											<span><format:price priceData="${entry.value.consumedAmount}"/>&nbsp;</span>
										</c:when>
										<c:otherwise>
											<span>&nbsp;</span>
										</c:otherwise>
							...
							<td class="column4">
								<c:choose>
									<c:when test="${not empty entry.value.balanceAmount}">
										<span><format:price priceData="${entry.value.balanceAmount}" hideZeroValue="true"/>&nbsp;</span>
									</c:when>
									<c:otherwise>
										<span>&nbsp;</span>
									</c:otherwise>
			...
			<tr>
				<th class="no-sort text-uppercase">
					<div class="checkbox checkbox-info selectchkbox">
						...
						<label for="contract-select-all-mobi"  id="contract-head-chck-label"><label:message messageCode="search.result.selectAll"/></label>
					</div>
				</th>
			 </tr>
		</thead>
						...
						<div class="display-table-cell" style="vertical-align:top">
							...
							<div class="display-table-row">
								<div class="display-table-cell">
									<a data-toggle="collapse" data-parent="#accordion" href="#collapse${mobstatus.count}" class="toggle-link panel-collapsed skyBlue ipadacctoggle">
					...
					<div id="collapse${mobstatus.count}" class="panel-collapse collapse">
						<div class="panel-body">
							<div class="sub-details-row">
								<div class="mob-view-label uppercase"><label:message messageCode="contract.itemPrice"/></div>
								<c:choose>
									<c:when test="${not empty contractEntry.contractProductPrice}">
							...
							<c:if test="${documentType == quantityBasedContract}">
								<div class="sub-details-row">	
									<div class="mob-view-label uppercase"><label:message messageCode="contract.unitOfMeasure"/></div>
									<c:choose>
										<c:when test="${not empty contractEntry.prodSalesUnit}">
								...
								<div class="sub-details-row">
									<div class="mob-view-label uppercase"><label:message messageCode="contract.contractQty"/></div>
									<c:choose>
										<c:when test="${not empty contractEntry.contractQty}">
								...
								<div class="sub-details-row">
									<div class="mob-view-label uppercase"><label:message messageCode="contract.consumed"/></div>
										<c:choose>
											<c:when test="${not empty contractEntry.consumedQty}">
								...
								<div class="sub-details-row">
									<div class="mob-view-label uppercase"><label:message messageCode="contract.balance"/></div>
									<c:choose>
										<c:when test="${not empty contractEntry.contractBalanceQty}">
							...
							<c:if test="${documentType == 'ZCV'}">
								<c:forEach items="${cntrctEntryMap}" var="entry"  varStatus="count">
									<c:if test="${contractEntry.productCode eq entry.key}">
										<div class="sub-details-row">
											<div class="mob-view-label uppercase"><label:message messageCode="contract.totalValue"/></div>
											<c:choose>
												<c:when test="${not empty entry.value.totalAmount}">
													<div><format:price priceData="${entry.value.totalAmount}" hideZeroValue="true"/>&nbsp;</div>
												</c:when>
												<c:otherwise>
													<div>&nbsp;</div>
												</c:otherwise>
										...
										<div class="sub-details-row">
											<div class="mob-view-label uppercase"><label:message messageCode="contract.consumedValue"/></div>
											<c:choose>
												<c:when test="${not empty entry.value.consumedAmount}">
													<div><format:price priceData="${entry.value.consumedAmount}"/>&nbsp;</div>
												</c:when>
												<c:otherwise>
													<div>&nbsp;</div>
												</c:otherwise>
										...
										<div class="sub-details-row">
											<div class="mob-view-label uppercase"><label:message messageCode="contract.contractBalance"/></div>
											<c:choose>
												<c:when test="${not empty entry.value.balanceAmount}">
													<div><format:price priceData="${entry.value.balanceAmount}" hideZeroValue="true"/>&nbsp;</div>
												</c:when>
												<c:otherwise>
													<div>&nbsp;</div>
												</c:otherwise>
...
<!-- Accordian for mobile ends here -->
	...
	<input type="hidden" value="${fn:length(contractEntryList)}" id="totalEntriesInContract"/>
	<c:if test="${contractData.totalItem gt fn:length(contractEntryList)}">
		<div class="sectionBlock clear ajaxLoad">
			<c:url  value="/my-account/contract/getContractDetails/${contractData.eccContractNum}" var="contractDetails"></c:url>
			<a style="" class="loadMore"  href="${contractDetails}?entryCount=${fn:length(contractEntryList)+30}">
				<label:message  messageCode="contract.loadMore" /></a> <span class="prodSmlfont"> <spring:message code="contract.page.30prods"/></span>
		</div>
	</c:if>

	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<input  type="button" class="btn btnclsactive pull-right contract-add-to-cart-bottom primarybtn addContractToCart" value= '<label:message messageCode="contract.detail.button.addtocart" />'/>
			<div class="showError">
				<input id="errorTemplateCart" class="tertiarybtn homeCartErrors" style="display:none" type="button" value='<spring:message code="homePage.errorDetails"/>' />
			</div>
			<div class="error_details" style="display:none;color: red;" > 
			<ul></ul></div>
			<%-- <home:errorDetails></home:errorDetails> --%>
		</div>
	</div>
					...
					<div class="modal-dialog modalcls">
						...
						<div class="modal-content popup">
								...
								<div class="panel panel-danger">
									<div class="panel-heading">
										...
										<table class="contract-popup-table">
											...
											<td><div class="info-text" id="popInfoText"></div>
							...
							<div class="modal-footer ftrcls">
								<a href="#" class="pull-left canceltxt" data-dismiss="modal" id="cancel-btn-addtocart1"><spring:message code="cart.common.cancel"/></a>
								<button type="button" class="btn btnclsactive" data-dismiss="modal" id="accept-btn-addtocart1" ><spring:message code="contract.page.accept"/></button>
							</div>
						</div>
					</div>
				</div>
