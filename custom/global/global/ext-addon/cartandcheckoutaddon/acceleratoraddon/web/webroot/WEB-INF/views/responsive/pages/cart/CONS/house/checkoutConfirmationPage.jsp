...
<template:page pageTitle="${pageTitle}">
	...
	<analytics:orderConfirmationAnalytics/>
	 ...
	 <!-- h1 : START -->   
		<h1><label:message messageCode="cart.confirmation.shoppingCart"/></h1>        	
  	 <!-- h1 : End -->              
	 ...
	 <div class="success"><p> <span class="strong"><label:message messageCode="cart.confirmation.thanksMessage"/></span><label:message messageCode="cart.confirmation.processMessage"/></p></div>    
    ...
    <div id="globalMessages">
     ...
     <!--Order Submission block starts-->
       <houseCart:confirmationPageHeader currentPage="orderConfirmationPage" orderData="${orderData}" orderType="${orderData.orderType}"/>
		...
		<div>
        <label:message messageCode="cart.common.totalWeight"/><b>&nbsp;${orderData.orderWeight}</b>&nbsp;<b>${orderData.orderWeightUOM}</b>&nbsp;&nbsp;&nbsp;
		<label:message messageCode="cart.common.totalVolume"/><b>&nbsp;${orderData.orderVolume}</b>&nbsp;<b>${orderData.orderVolumeUOM}</b>
		</div>
		...
		<houseCart:confirmationPageEntries orderData="${orderData}" />
          <div class="sectionBlock buttonWrapperWithBG borDer">
			...
			<p><span><label:message messageCode="cart.validate.cartTotal.subTotal"/></span> <span class="jnjID"><format:price priceData="${orderData.totalPrice}" /></span></p>		
			<c:if test="${cartData.totalFees.value ne '0.0'}">
			
			<p><span><label:message messageCode="cart.validate.cartTotal.tax"/></span> <span class="jnjID"> <format:price priceData="${orderData.totalTax}"/></span></p>
			</c:if>
			<p class="totalSum"><span><label:message messageCode="cart.validate.cartTotal.totals"/></span> <span class="jnjID"><format:price priceData="${orderData.totalPrice}"/></span></p>
	   		</div>
      	  </div>
		<div class="sectionBlock buttonWrapperWithBG continueShopping shopStep4">
			<checkout:orderConfirmationButtons orderData="${orderData}" />
