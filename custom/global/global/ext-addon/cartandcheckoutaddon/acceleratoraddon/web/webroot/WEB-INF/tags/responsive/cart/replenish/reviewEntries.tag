...
<!-- replenish/reviewEntries.tag -->
<div class="mainbody-container">
	<c:choose>
		<c:when test="${splitCart}">
			<c:forEach items="${jnjCartDataList}" var="jnjCartData" varStatus="count">
				<replenishCart:multiReviewEntries jnjCartData="${jnjCartData}" globalCount="${count.count}" />
			</c:forEach>
			<div class="sectionBlock buttonWrapperWithBG borDer smarLeft">
				<div class="txtRight">
					<replenishCart:cartTotals />
				</div>
			</div>
		</c:when>
		<c:otherwise>
			<replenishCart:singleReviewEntry />
		</c:otherwise>
	</c:choose>
</div>