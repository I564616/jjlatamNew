...
<form:form id="UpdateMultipleEntriesInCartForm" action="updateAll"
	commandName="UpdateMultipleEntriesInCartForm">
</form:form>
...
<%-- 								code="cart.review.entry.total" /></th> --%>
				...
				<tbody>
					...
					<c:forEach items="${cartData.entries}" var="entry"  varStatus="count">
						...
						<td>${count.count}</td>
							<td class="text-left"><standardCart:productDescriptionBeforeValidation	entry="${entry}" priceError="${priceValidationErrorMsg}" kitError="${kitValidationErrorMsg}" showRemoveLink="true" showStatus="false" /></td>
							<td class="valign-middle">
								<div class="cart-update-link">
									<a href="javascript:void();" id="quantity${entry.entryNumber}_update"
										entryNum="${entry.entryNumber}" class="qtyUpdateTextBox1"><spring:message
              ...
              <td>
                                <p class="textLeft">
                                  <span class="textBlack">
                                    <fmt:formatDate value="${deliverySch.deliveryDate}" pattern="MM/dd/yyyy"/>
                                  </span>
                                </p> 
               </td>
              <td>
                <c:choose>
                  ...
                  <div class="text-uppercase"> <spring:message code="product.variants.available" /> </div>  
                  </c:when>
                  <c:otherwise>
			...
			<table id="datatab-mobile"
				...
				<tbody>
					<c:forEach items="${cartData.entries}" var="entry"
						...
						<tr>
							<td class="text-left">
								<standardCart:productDescriptionBeforeValidation entry="${entry}" rowcount="${count.count}" showRemoveLink="true" showStatus="false" />
								...
								<div id="mobi-collapse${count.count}"
									...
									<form:form id="updateCartForm${entry.entryNumber}_mobile" action="${cartUpdateFormAction}" method="post"
										...
										<input type="hidden" name="productCode" value="${entry.product.code}" />
										<input type="hidden" name="initialQuantity" value="${entry.quantity}" />
									<div class="cart-update-link">  <a href="#" id="quantity${entry.entryNumber}_mobile_update" entryNum="${entry.entryNumber}" class="qtyUpdateTextBox1 click_updateBtn_mobile" ><spring:message code="cart.review.productDesc.updateItem"/></a></div> 
										<ycommerce:testId code="cart_product_quantity">
							...
							<c:if test="${not empty valueObject.itemCategory}">
								<tr class="noborder" id="freeGood${entry.product.code}">
									...
									<td>
										<div class="text-center">
											<c:url value="/cart/update" var="cartUpdateFormAction" />
											<form:form id="updateCartForm${entry.entryNumber}_mobile"
												action="${cartUpdateFormAction}" method="post"
												commandName="updateQuantityForm${entry.entryNumber}">
												<input type="hidden" name="entryNumber"
													value="${entry.entryNumber}" />
