		...
		<div class="col-lg-6 col-md-6 col-sm-7 col-xs-12">
			...
			<div class="orderType-holder">
				...
				<c:choose>
					...
					<c:otherwise>
						<button type="button" class="btn btnclsactive replenishValidateSaveupdate"><spring:message code="cart.review.progressBar.validate"/></button>
					</c:otherwise>
				</c:choose>
				<!-- No charge Order AAOL-3392 -->
			</div>
...
<!-- Error Messages Block - Header Level start-->
...
<c:if test="${not empty accInfoMsgs}">	
		<c:forEach items="${accInfoMsgs}" var="msg">
			...
			<div class="panel panel-danger">
				<div class="panel-heading">				
				<span class="glyphicon glyphicon-ban-circle" style="margin-right:5px"></span><p><spring:message code="${msg.code}"/></p>
			</div>
			</div>
		</div>	
		</c:forEach>	
...
<c:if test="${not empty accErrorMsgs}">
	...
	<c:forEach items="${accErrorMsgs}" var="msg">
		<div class="panel-group">
			<div class="panel panel-danger">
				<div class="panel-heading">
					<span class="glyphicon glyphicon-ban-circle" style="margin-right:5px"></span><spring:message code="${msg.code}" arguments="${msg.attributes}"/>
				</div>
			</div>
		</div>	
	</c:forEach>	
...
<!-- Error Messages Block - Header Level end-->
...
<div class="mainbody-container">
	...
	<!-- No charge Order AAOL-3392 -->
	<replenishCart:cartPageHeader currentPage="cartPage"/>
		...
		<div class=" Subcontainer visible-xs hidden-lg hidden-sm hidden-md">
					...
					<c:forEach items="${cartData.entries}" var="entry"  varStatus="count">
						<tr>
							...
							<td class="text-left">
								<standardCart:productDescriptionBeforeValidation entry="${cartData.entries[fieldLength - count.count]}" errorCode="${validationErrorMsg}"  showRemoveLink="true" showStatus="false" rowcount="${count.count}" priceError="${priceValidationErrorMsg}"/>
