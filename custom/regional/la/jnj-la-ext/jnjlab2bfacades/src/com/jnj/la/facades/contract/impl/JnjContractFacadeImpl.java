 ...
 *
 */
package com.jnj.la.facades.contract.impl;
 ...
 * @author komal.sehgal
 */
public class JnjContractFacadeImpl implements JnjContractFacade
{
	...
	@Autowired
	protected JnjContractService jnjContractService;
	...
	public List<JnjContractData> getContracts()
	{
		...
		{
			LOGGER.debug(
					Jnjb2bCoreConstants.Logging.CONTRACTS_FACADE + " - " + Jnjb2bCoreConstants.Logging.CONTRACTS_GET_CONTRACTS_METHOD
		...
		final List<JnjContractData> JnjContractDataList = new ArrayList<>();
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.jnj.facades.contract.JnjContractFacade#getPagedOrderTemplates(de.hybris.platform.commerceservices.search.
	 * pagedata .PageableData, java.lang.String, java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
			...
			final String searchParameter, final String sortByCriteria, final String selectCriteria)
	{
		...
		{
			LOGGER.debug(
					Jnjb2bCoreConstants.Logging.CONTRACTS_FACADE + " - " + Jnjb2bCoreConstants.Logging.CONTRACT_GET_PAGED_CONTRACTS
							+ "-" + Logging.BEGIN_OF_METHOD + JnJCommonUtil.getCurrentDateTime());
		}
		final SearchPageData<JnjContractModel> contractResults = jnjContractService.getPagedContracts(pageableData,
				searchByCriteria, searchParameter, sortByCriteria, selectCriteria);
		...
		final SearchPageData<JnjContractData> result = new SearchPageData<>();
	...
	public boolean checkAllActiveContracts() throws BusinessException
	{
		...
		while (!endOfBatch)
		{
			final List<JnjContractModel> activeContracts = new ArrayList<>();
			SearchResult<JnjContractModel> searchResult = null;
			List<JnjContractModel> fetchedresults = null;
			...
			try
			{
				...
				searchResult = jnjContractService.getActiveContracts(CONTRACT_STATUS_ACTIVE, startIndex);
			}
			...
			fetchedresults = searchResult.getResult();

			for (int i = 0; i < fetchedresults.size(); i++)
			{
				if (fetchedresults.get(i).getActive().equals(Boolean.TRUE))
				{
					activeContracts.add(fetchedresults.get(i));
				}
			 ...
			 */
			startIndex = startIndex - startIndexShift;
			/*
	...
	public boolean checkAllInActiveContracts() throws BusinessException
	{
		...
		while (!endOfBatch)
		{
			...
			try
			{
				searchResult = jnjContractService.getActiveContracts(CONTRACT_STATUS_IN_ACTIVE, startIndex);
			}
			catch (final BusinessException businessException)
			{
			...
			if (!(IterableUtil.isNullOrEmpty(activeContracts)))
			{
				...
				for (final JnjContractModel jnjContractModel : activeContracts)
				{
					...
					if (("A").equalsIgnoreCase(entryStatus) || ("B").equalsIgnoreCase(entryStatus))
					{
						contractStatus = true;
						LOGGER.info(Jnjb2bCoreConstants.Logging.CONTRACTS_FACADE + Logging.HYPHEN + METHOD_NAME + Logging.HYPHEN
	...
	public boolean checkActiveContractsForEntry() throws BusinessException
	{
		...
		while (!endOfBatch)
		{
			...
			try
			{
				searchResult = jnjContractService.getActiveContractsForEntry(CONTRACT_STATUS_ACTIVE, startIndex);
			}
			catch (final BusinessException businessException)
			{
				LOGGER.info(Jnjb2bCoreConstants.Logging.CONTRACTS_FACADE + Logging.HYPHEN + GET_ALL_ACTIVE_CONTRACTS + Logging.HYPHEN
			...
			if (!(IterableUtil.isNullOrEmpty(activeContracts)))
			{
				...
				for (final JnjContractModel jnjContractModel : activeContracts)
				{
					if (LOGGER.isDebugEnabled())
					{
						LOGGER.debug(Jnjb2bCoreConstants.Logging.CONTRACTS_FACADE + Logging.HYPHEN + GET_ALL_ACTIVE_CONTRACTS
								+ Logging.HYPHEN + "Contracts with ID [" + jnjContractModel.getECCContractNum()
								+ "] is expired. Changin status to [Inactive]");
					}
