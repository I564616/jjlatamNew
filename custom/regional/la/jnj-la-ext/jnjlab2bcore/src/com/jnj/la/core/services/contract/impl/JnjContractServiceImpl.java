 ...
 *
 */
package com.jnj.la.core.services.contract.impl;
 ...
 * @author komal.sehgal
 */
public class JnjContractServiceImpl implements JnjContractService
{
	...
	/**  */
	private static final String GET_ACTIVE_CONTRACTS = "getActiveContracts()";

	/** The Constant LOG. */
	private static final Logger LOGGER = Logger.getLogger(JnjContractServiceImpl.class);

	/** The Constant METHOD_SAVE_ITM_CONTRACT_MODEL. */
	private static final String METHOD_SAVE_ITM_CONTRACT_MODEL = "saveItmContractModel()";

	/** The Constant METHOD_GET_IMT_CONTRACT_MODEL. */
	private static final String METHOD_GET_IMT_CONTRACT_MODEL = "getJnjImtContractModel()";

	/** The Constant METHOD_GET_IMT_CONTRACTS_MODEL. */
	private static final String METHOD_GET_IMT_CONTRACTS_MODEL = "getJnjImtContractModels()";
	...
	@Autowired
	private JnjContractDao jnjContractDao;
	...
	{
		final String METHOD_NAME = "getContracts()";
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - "
					+ Jnjlab2bcoreConstants.Logging.CONTRACTS_GET_CONTRACTS_METHOD + "-" + Logging.BEGIN_OF_METHOD
		...
		{
			JnjGTCoreUtil.logWarnMessage("JnjContractServiceImpl  Exception", METHOD_NAME, "Message", exception,
					JnjContractServiceImpl.class);
			LOGGER.error(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - "
					+ Jnjlab2bcoreConstants.Logging.CONTRACTS_GET_CONTRACTS_METHOD + "-" + "No Default B2B Unit for currrent User"
					+ JnJCommonUtil.getCurrentDateTime());
		}
	...
	public JnjContractModel getContractDetailsById(final String eCCContractNum)
	{
		...
		{
			LOGGER.debug(
					Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - " + Jnjlab2bcoreConstants.Logging.CONTRACT_DETAILS_BY_ID
	...
	public boolean isContractBid(final String contracrId)
	{
		...
		if (StringUtils.isNotEmpty(contracrId))
		{
			...
			if (null != jnjContractModel && StringUtils.equalsIgnoreCase(jnjContractModel.getContractOrderReason(),
	...
	{
		final String METHOD_NAME = "saveContract()";
		boolean recordStatus = false;
		String errorMessage = null;
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(
					Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN + Jnjlab2bcoreConstants.Logging.SAVECONTRACT
	...
	/*
	 * @Override public boolean saveItmContractModel(final JnjImtContractModel jnjImtContractModel) throws
	 * BusinessException { boolean recordStatus = false; if (LOGGER.isDebugEnabled()) {
	 * LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN + METHOD_SAVE_ITM_CONTRACT_MODEL +
	 * Logging.HYPHEN + Logging.BEGIN_OF_METHOD + JnJCommonUtil.getCurrentDateTime()); } if (null != jnjImtContractModel)
	 * { try { modelService.saveAll(jnjImtContractModel); recordStatus = true; if (LOGGER.isDebugEnabled()) {
	 * LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN + METHOD_SAVE_ITM_CONTRACT_MODEL +
	 * Logging.HYPHEN + "Successfully saved Intermediate Contract Model with ECCContractNum: " +
	 * jnjImtContractModel.getContractNoEcc()); } } catch (final ModelSavingException exception) {
	 * LOGGER.error(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN + METHOD_SAVE_ITM_CONTRACT_MODEL +
	 * Logging.HYPHEN + "Error occured while saving Intermediate Contract Model with ECCContractNum: " +
	 * jnjImtContractModel.getContractNoEcc() + exception); throw new
	 * BusinessException("Error occured while saving Intermediate Contract Model with ECCContractNum: " +
	 * jnjImtContractModel.getContractNoEcc()); } } if (LOGGER.isDebugEnabled()) {
	 * LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN + METHOD_SAVE_ITM_CONTRACT_MODEL +
	 * Logging.HYPHEN + Logging.END_OF_METHOD + JnJCommonUtil.getCurrentDateTime()); } return recordStatus; }
	 */
	...
	{
		String sortByCriteriaString = sortByCriteria;
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - "
					+ Jnjlab2bcoreConstants.Logging.CONTRACT_GET_PAGED_CONTRACTS + "-" + Logging.BEGIN_OF_METHOD
					+ JnJCommonUtil.getCurrentDateTime());
		}
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - "
					+ Jnjlab2bcoreConstants.Logging.CONTRACT_GET_PAGED_CONTRACTS + "-" + Logging.END_OF_METHOD
					+ JnJCommonUtil.getCurrentDateTime());
		}
		if (sortByCriteriaString == null)
		{
			sortByCriteriaString = Jnjb2bCoreConstants.Contract.SORT_BY_CONTRACT_NUMBER_DESC;
		}
		return jnjContractDao.getPagedContracts(pageableData, searchByCriteria, searchParameter, sortByCriteriaString,
				selectCriteria);
	}
	...
	/*
	 * @Override public List<JnjImtContractModel> getJnjImtContractModels(final String iDocNum, final RecordStatus
	 * recordStatus, final String eCccontractNum, final String fileName) { if (LOGGER.isDebugEnabled()) {
	 ...
	 *
	 * List<JnjImtContractModel> jnjImtContractModelList = null; final JnjImtContractModel tempJnjImtContractModel = new
	 * JnjImtContractModel();
	 *
	 * String logMessage = null;
	 *
	 * if (null != recordStatus) { tempJnjImtContractModel.setRecordStatus(recordStatus); logMessage = " RecordStatus ["
	 * + recordStatus.toString() + "] "; } if (StringUtils.isNotEmpty(iDocNum)) {
	 * tempJnjImtContractModel.setIdocNumber(iDocNum); logMessage = StringUtils.isEmpty(logMessage) ? " IDOC Number [" +
	 * iDocNum + "] " : logMessage.concat(", IDOC Number [" + iDocNum + "] ");
	 *
	 * } if (StringUtils.isNotEmpty(eCccontractNum)) { tempJnjImtContractModel.setContractNoEcc(eCccontractNum);
	 * logMessage = StringUtils.isEmpty(logMessage) ? " Contract Number [" + eCccontractNum + "] " : logMessage
	 * .concat(", Contract Number [" + eCccontractNum + "] "); } if (StringUtils.isNotEmpty(fileName)) {
	 * tempJnjImtContractModel.setFileName(fileName); logMessage = StringUtils.isEmpty(logMessage) ? " File Name[" +
	 * fileName + "] " : logMessage.concat(", File Name [" + fileName + "] "); }
	 *
	 * try { jnjImtContractModelList = flexibleSearchService.getModelsByExample(tempJnjImtContractModel); if
	 * (!jnjImtContractModelList.isEmpty() && LOGGER.isDebugEnabled()) { LOGGER.debug(Logging.CONTRACTS_SERVICE +
	 * Logging.HYPHEN + METHOD_GET_IMT_CONTRACT_MODEL + Logging.HYPHEN + "JnjImtContractModels with Properties" +
	 * logMessage + " found in Hybris. Returning Existing Models"); } else if (LOGGER.isDebugEnabled()) {
	 * LOGGER.debug(Logging.CONTRACTS_SERVICE + Logging.HYPHEN + METHOD_GET_IMT_CONTRACT_MODEL + Logging.HYPHEN +
	 * "JnjImtContractModels with Properties" + logMessage + " not found in Hybris. Returning Empty List."); } } catch
	 * (ModelNotFoundException | IllegalArgumentException exception) { LOGGER.error(Logging.CONTRACTS_SERVICE +
	 * Logging.HYPHEN + METHOD_GET_IMT_CONTRACT_MODEL + Logging.HYPHEN + "JnjImtContractModels with Properties" +
	 * logMessage + " not found in Hybris. Returning Null - " + exception); }
	 *
	 * if (LOGGER.isDebugEnabled()) { LOGGER.debug(Logging.CONTRACTS_SERVICE + Logging.HYPHEN +
	 * METHOD_GET_IMT_CONTRACTS_MODEL + Logging.HYPHEN + Logging.END_OF_METHOD + Logging.HYPHEN +
	 * System.currentTimeMillis()); } return jnjImtContractModelList;
	 *
	 * }
	 */
	...
	public JnjUomDTO getContractDelUomMapping(final JnJProductModel jnjProductModel, final UnitModel unitModel)
	{
		...
		if (null != unitModel && CollectionUtils.isNotEmpty(jnjProductModel.getUomDetails()))
		{
			for (final JnjUomConversionModel jnjUomConvModel : jnjProductModel.getUomDetails())
			{
				...
				if (jnjUomConvModel.getUOM().equals(unitModel))
				{
					...
					contractDelUOM.setSalesUnitsCount(
							numSUOM / jnjUomConvModel.getNumerator().intValue() / jnjUomConvModel.getDenominator().intValue());
				}
			}
		}
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(
					"getCustDelUomMapping()" + Logging.HYPHEN + Logging.END_OF_METHOD + Logging.HYPHEN + System.currentTimeMillis());
		}
	...
	@Override
	public SearchResult<JnjContractModel> getActiveContracts(final String status, final int startIndex) throws BusinessException
	{
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(Logging.CONTRACTS_SERVICE + Logging.HYPHEN + GET_ACTIVE_CONTRACTS + Logging.HYPHEN + Logging.BEGIN_OF_METHOD
					+ Logging.HYPHEN + System.currentTimeMillis());
		}

		final SearchResult<JnjContractModel> activeContractListResult = jnjContractDao.getAllContractWithStatus(status, startIndex);
	...
	public boolean isProductsExistsInContract(final List<String> productCodes)
	{
		...
		{
			LOGGER.debug(
					Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - " + Jnjlab2bcoreConstants.Logging.CONTRACT_PRODUCT_EXIST
							+ "-" + Logging.BEGIN_OF_METHOD + JnJCommonUtil.getCurrentDateTime());
		}
		...
		if (StringUtils.isNotEmpty(contractId))
		{
			...
			final List<String> productsInContract = new ArrayList<>();
			for (final JnjContractEntryModel jnjContractEntryModel : jnjContractModel.getJnjContractEntries())
			{
				if (!StringUtils.isEmpty(jnjContractEntryModel.getProduct().getCode()))
				{
					...
					try
					{
						...
						{
							LOGGER.warn(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + Logging.HYPHEN
									+ Jnjlab2bcoreConstants.Logging.CONTRACT_PRODUCT_EXIST + Logging.HYPHEN
	...
	public boolean isContractrValidForProducts(final List<String> productCodes)
	{
		...
		if (StringUtils.isNotEmpty(contractId))
		{
			...
			final List<JnJProductModel> activeProductList = new ArrayList<>();
			...
			activeProductListLoop: for (final JnJProductModel jnJProductModel : activeProductList)
			{
				...
				{
					//if (StringUtils
					//		.equalsIgnoreCase(((JnJLaProductModel)jnjContractEntryModel.getProduct()).getCatalogId(), ((JnJLaProductModel)jnJProductModel).getCatalogId()))
					if (StringUtils.equalsIgnoreCase((jnjContractEntryModel.getProduct().getCatalogId()),
							(jnJProductModel.getCatalogId())))
					{
						if (!isContractEntryActive(jnjContractEntryModel))
						{
							LOGGER.info(Jnjlab2bcoreConstants.Logging.CONTRACTS_SERVICE + " - "
									+ Jnjlab2bcoreConstants.Logging.CONTRACT_PRODUCT_EXIST + "-" + "Contract Entry For Prodcut ["
	 ...
	 */
	@Override
	public SearchResult<JnjContractModel> getActiveContractsForEntry(final String CONTRACT_STATUS_ACTIVE, final int startIndex)
			throws BusinessException
	{
		...
		final SearchResult<JnjContractModel> activeContractListResult = jnjContractDao
				.getAllContractForEntry(CONTRACT_STATUS_ACTIVE, startIndex);
		if (LOGGER.isDebugEnabled())
		{
