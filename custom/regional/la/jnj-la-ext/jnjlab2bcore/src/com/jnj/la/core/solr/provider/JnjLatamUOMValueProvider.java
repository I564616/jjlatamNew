...
public class JnjLatamUOMValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{
				...
				for (final LanguageModel language : indexConfig.getLanguages())
				{
					...
					try
					{
						...
						if (null != productModel.getDeliveryUnitOfMeasure())
						{
							...
							deliveryUnitCode = productModel.getDeliveryUnitOfMeasure().getCode();
						}
						else
						{
							deliveryUnit = "pieces";
						...
						unitOfMeasure = salesUnit + deliminator + saleItemQuantity + deliminator + deliveryUnit + deliminator
								+ deliveryUnitCode;
