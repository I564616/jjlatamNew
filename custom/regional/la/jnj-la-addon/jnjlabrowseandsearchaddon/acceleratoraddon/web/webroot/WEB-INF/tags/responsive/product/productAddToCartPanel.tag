...
<c:if test="${(product.discontinued || !product.salesRepCompatibleInd)}">
...
<form id="prodAddToCartForm" class="prod_list_form"	action="<c:url value="/cart/add"/>" method="post"></form>
