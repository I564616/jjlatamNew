	...
	<div id="templatedetailpage">
		<div class="row">
			<div class="col-xs-12 col-md-12 col-sm-12 col-lg-12">
				...
				<div class="row content">
					<div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">${templateEditForm.templateName}</div>
					<div class="col-lg-4 col-md-4 col-sm-3 col-xs-12">
						<!-- 	<input type="button" value="Add All to Cart" class="primarybtn templateDetailAddToCart"/> -->
						<button type="button" class="btn btnclsactive pull-right primarybtn laAddToCartOrderTemplate">
							<spring:message code="template.detail.addToCart" />
						</button>
					</div>
				</div>
				<div class="error hide" id="addToCartErrorLayer">
					<p></p>
				</div>
			</div>
		</div>
			...
			<div class="col-lg-12 col-md-12">
				<div class="table-padding">
					<div class="row editDeletepanel">
						<c:url
							value="/templates/templateDetail/deleteTemplate/${templateId}"
							var="templateEditUrl"/>
						<form:form action="${templateEditUrl}"
							modelAttribute="templateEditForm" id="templateEditForm"
							method="POST">
							<span class="textBlack templateNumber" style="display:none">${templateId}</span>
							<form:input path="templateNumber" id="hddnTemplateNumber" type="hidden"/>
							<span style="display:none">
							    <input type="text" name="templateName" id="templateName" value="${templateEditForm.templateName}"/>
							</span>
							<c:if test="${user.uid eq templateEditForm.author}">
								<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 tabletbtnwidth">
									<button type="button"
										class="btn btnclsnormal pull-left editTemplatebtn">
										<spring:message code="template.detail.editTemplate"/>
									</button>
									<button type="button"
										class="btn btnclsnormal pull-left deleteTemplatebtn tertiarybtn deleteTemplate">
										<spring:message code="template.detail.deleteTemplate"/>
								    </button>
								</div>
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-6 tabletShareTxt">
									<div class="checkbox checkbox-info checkboxmargin pull-right styled">
										<form:checkbox name="shareWithAccountUsers"
											id="shareWithAccountUsers" path="shareWithAccountUsers"
											class=" styled tertiarybtn saveTemplate"/>
										<label for="shareWithAccountUsers">
										    <spring:message code="template.detail.share.with.users"/>
										</label>
									</div>
								</div>
							</c:if>
							<div class="col-lg-3 col-md-3 col-sm-3 col-xs-6 downloadbuttons">
								<div class="pull-right downloadlinks downloadbuttons sectionBlock ">
									...
									<a href="#" class="tertiarybtn excel"> &nbsp;
									...
									<tr>
										...
										<td class="text-left verticalmiddle">
										    <span class="Tabsubtxt">
										        <input type="hidden"
												    value="${productEntry.refVariant.saleableInd && productEntry.salesRepDivisionCompatible}"
												    class="saleableInd" id="${productEntry.refVariant.code}"
												...
												<p class="secondline boldtext">${productEntry.catalogId}</p>
												...
												<c:if test="${productEntry.refVariant.productVolume ne null}">
													<span class="descSmall">
														...
														${productEntry.refVariant.productVolume}
													</span>
												</c:if>
											...
											<div class="row">
												<form class="form-inline" role="form">
													<div class="form-group pull-right ">
														<label for="email">
														    <spring:message code="template.create.quantity"></spring:message>
														</label>
														<input type="text"
															value="${productEntry.qty}"
															class="form-control txtWidth " disabled="disabled">
													</div>
												</form>
											</div>
					...
					<c:forEach items="${orderTemplateDetail}" var="productEntry" varStatus="count">
					    <c:set var="productCount" value="${count.count}"/>
					    <input type="hidden" value="${productEntry.catalogId}" class="form-control txtWidth"
					        id="productId${count.index}" readonly="readonly"/>
					    <input type="hidden" value="${productEntry.qty}"
					        class="halign2 column3Input pull-right descMid form-control txtWidth"
					        id="quantity${count.index}" name="qty"/>
                    </c:forEach>
					<input type="hidden" name="numbrOfProductLines" id="numberOfProductLines" value="${productCount}"/>
										...
										<c:choose>
											<c:when test="${!productEntry.refVariant.isProdViewable}">
												<product:productPrimaryImage
													product="${productEntry.refVariant}" format="cartIcon" />
											</c:when>
											<c:otherwise>
												<a href="${productUrl}"><product:productPrimaryImage
													product="${productEntry.refVariant}" format="cartIcon" /></a>
											</c:otherwise>
										</c:choose>
										<input type="hidden"
											value="${productEntry.refVariant.saleableInd && productEntry.salesRepDivisionCompatible}"
											...
											<div class="Tabsubtxt text-left verticalmiddle">
												...
												<c:if test="${productEntry.refVariant.productWeight ne null}">
													<p class="thirdline pull-left marginleft30px">
														<strong>
														    <spring:message code="template.detail.weight" />:
														</strong> ${productEntry.refVariant.productWeight}
													</p>
												</c:if>
											...
											<div style="width: 30%; float: left; margin-top: 5px;">
												<form class="form-inline margintop5px" role="form">
													<div class="form-group ">
														...
														<input type="text" value="${productEntry.qty}" class="form-control txtWidth">
													</div>
												</form>
											</div>
											<input type="hidden" name="numbrOfProductLines" id="numberOfProductLines" value="10"/>
											<input type="hidden" value="${product.baseMaterialNumber}"
											    class="form-control txtWidth" id="productId${count.index}" readonly="readonly"/>
											<input type="hidden" value="${productEntry.qty}"
											    class="halign2 column3Input pull-right descMid form-control txtWidth"
											    id="quantity${count.index}" name="qty"/>
											<div class="viewprice">
				...
				<div class="row basecontent">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<button type="button" class="btn btnclsactive pull-right laAddToCartOrderTemplate">
							<spring:message code="template.detail.addToCart" />
						</button>
					</div>
				</div>
		...
		<div class="modal fade jnj-popup" id="confirmationPopup" role="dialog">
			<div class="modal-dialog modalcls modal-md">
				<!-- Modal content-->
				<div class="modal-content" id="Confirmationext"></div>
			</div>
		</div>
		<!--End of Modal pop-up-->
