 ...
 *
 */
public class JnjLatamContractPdfView extends AbstractPdfView
{
			...
			final HttpServletRequest request, final HttpServletResponse response) throws Exception
	{
		try
		{
			...
			final String fileName = getJnjCommonFacadeUtil().getMessageFromImpex("contract.myContracts") + ".pdf";
			...
			PdfPTable imageTable = new PdfPTable(1); // 3 columns.
			...
			headerCell.setPhrase(new Phrase(getJnjCommonFacadeUtil().getMessageFromImpex("text.contract.pdfexcel.list"),
			...
			if (results != null && !results.isEmpty())
			{
				...
				for (final JnjContractData data : results)
				{
					...
					orderTable.addCell(
							createValueCell((data.getEndDate() != null) ? dateFormat.format(data.getEndDate()).toString() : emptyField));
		...
		{
			JnjGTCoreUtil.logErrorMessage("Latam Contract PDF Download", "buildPdfDocument()", "Error While downloading PDF", e,
					JnjLatamContractPdfView.class);
		}
	}
