...
<c:choose>
	...
	<c:otherwise>
		<a href="${productUrl}" class="imgprop1" data="${productCode}"><product:productPrimaryImage product="${entry.product}" format="cartIcon" /></a>
	</c:otherwise>
</c:choose>
...
<h4 title="${entry.product.name}">
	 ...
	 <p class="secondline"><spring:message code="cart.review.productDesc.jnJID"/><span class="strong">${entry.catalogId}</span>
	</p>
	<p class="secondline">
