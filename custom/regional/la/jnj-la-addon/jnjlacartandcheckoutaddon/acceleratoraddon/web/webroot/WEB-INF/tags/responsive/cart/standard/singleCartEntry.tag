...
<div class="row">
	<div class="col-lg-12 col-md-12">
		<div class=" d-none d-sm-block tableshadow">
			<div class="dateselectordropdown">
				<div class="dateselector">
					<div class="setshipping">
						<spring:message code="cart.shipping.RequestedDeliveryDate" />
					</div>
					<input type="hidden" id="hidden-sessionLang"
						value="${sessionlanguagePattern}" />
					<c:choose>
						<c:when test="${!checkoutoption}">
							<div class="input-group marginleft20">
								<input id="date-picker-head" name="toDate"
									placeholder="<spring:message code="cart.shipping.SelectDate" />"
									class="date-picker form-control requestDeliveryDataPicker"
									type="text" readonly
									value="<fmt:formatDate pattern="${sessionlanguagePattern}" value="${cartData.entries[0].expectedDeliveryDate}" />">
								<label for="date-picker-head" class="input-group-addon btn" style="border: 1px solid #ccc;border-left: 0px;"><span
									class="bi bi-calendar3"></span> </label>
							</div>

						</c:when>
						<c:otherwise>

							<select class="selectpicker shippingMethodSelect"
			...
			<table id="ordersTable"
				class="table table-bordered table-striped lasorting-table header-content-table"
				...
				<td class="hidden"></td>
				<td class="hidden"></td>
				<td colspan="5" class="dateselector">
					<div class="setshipping">
						<spring:message code="cart.shipping.RequestedDeliveryDate" />
					</div> 
					<c:choose>
										...
										<div class="input-group marginleft20">
												<input id="date-picker-head" name="toDate"
													placeholder="Select date" class="date-picker form-control"
													type="text" readonly value="<fmt:formatDate pattern="MM/dd/yyyy" value="${cartData.entries[0].shippingDate}" />"> <label for="date-picker-head"
					...
					<c:forEach items="${cartData.entries}" var="entry"
						...
						<tr id="orderentry-${count.count}">
							<td class="text-left shipmentPdtDetails"><standardCart:productDescriptionBeforeValidation
										...
										<c:when test="${!checkoutoption}">
											<div class="input-group  margintop10">
												<input data="${entry.entryNumber}"
													id="date-picker-${globalCount}${count.count}" name="toDate"
													placeholder="<spring:message code="cart.shipping.SelectDate" />"
													class="date-picker form-control date-picker-body requestDeliveryDataPicker selectExpectedDeliveryDate"
													type="text" readonly
													value="<fmt:formatDate pattern="${sessionlanguagePattern}" value="${entry.expectedDeliveryDate}" />">
												<label for="date-picker-${globalCount}${count.count}"
													class="input-group-addon btn" style="border: 1px solid #ccc;border-left: 0px;"><span
													class="bi bi-calendar3"></span> </label>
											</div>
										</c:when>
										<c:otherwise>
				...
				<table class="total-summary-table">
					<tr>
						<td class="total-summary-label"><spring:message
								code="cart.shipping.SubTotal" /></td>
